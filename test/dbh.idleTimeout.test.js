...
describe('dbh idleTimeout forwarding (BDL-3565)', () => {
	...
	// AC2 (a) -- the key must be ABSENT, mirroring the existing
	// minimumIdle/acquireTimeout assertion in MySQLDialect.createPool-bounds.test.js
	it('omits idleTimeout entirely when unset, so the dialect default applies', async () => {
		const pools = await capturePools({ readonlyNodes: RO_NODES });

		expect(pools).to.have.lengthOf(2);
		pools.forEach((p) => {
			expect(p).to.not.have.property('idleTimeout');
		});
	});
