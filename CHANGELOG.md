...
- **MySQL: double-quoted string literals are normalized to single quotes so the same SQL means the same thing under `ANSI_QUOTES`.** MySQL was the only dialect with no SQL normalization pass — `MySQLDialect.transformSql` was a literal identity function — so caller-authored SQL like `... LIKE "upm%"` reached the server byte-for-byte. Under default `sql_mode` that is a string; under `ANSI_QUOTES` it is an **identifier**, so the query succeeds on a primary and dies with `ERROR 1054 unknown column 'upm%'` on an `ANSI_QUOTES` read replica — routing-dependent, intermittent, and with an error message that points the reader at the schema rather than at quoting. New `lib/sql-transform/MySQLSqlTransformer.js` lexes the statement and rewrites every double-quoted string literal, **decoding out of the double-quote context and re-encoding into the single-quote one** rather than string-replacing across the encoding boundary (a naive escape corrupts `""` — one `"` in double-quote context, two in single — with no error).
  ...
  The rewrite **splices the original SQL source**, replacing only the double-quoted runs, rather than parsing and reserializing the statement. That matters for two reasons. Comments survive byte-for-byte — a serializer drops them and has no way to put them back, so an earlier cut of this fix had to skip comment-bearing SQL wholesale, which left a real DML query carrying both a comment and a double-quoted literal **still broken on `ANSI_QUOTES`**. And there is no parser in the path to reject anything, so SQL that `node-sql-parser` cannot parse at all — a reserved word used as an identifier, `... WHERE key LIKE "upm%"` — is fixed too. Whitespace and keyword case are the caller's and come back unchanged.

  A `"` inside a comment, a single-quoted string, or a backticked identifier is prose or content, not a literal, and is left alone. Still returned **byte-identical**: SQL with no `"` at all, any statement whose leading keyword is not `SELECT`/`INSERT`/`REPLACE`/`UPDATE`/`DELETE`, SQL with an unterminated string or block comment, and SQL where nothing needed rewriting. `--` only starts a comment when whitespace follows it, per MySQL — `a--1` is `a - (-1)`, and mis-lexing it would silently delete a predicate.

  DDL stays excluded for a **text-matching** reason, not a semantic one: `meta->>'valence'` means the same as `meta->>"valence"`, but it no longer matches the index definition MySQL reports back, so `schemaSync` would drop and recreate the index on every run.

  Known limitation, by design: SQL authored *for* `ANSI_QUOTES`, where `"col"` is meant as an identifier, is indistinguishable from a string literal here and would be rewritten to the constant `'col'`. That is already how MySQL reads it under default `sql_mode`, so it is a no-op for SQL written against a default-mode primary — but do not feed this ANSI-authored SQL.

### Added

- 38 tests in `lib/sql-transform/test/MySQLSqlTransformer.test.js`, including canaries for every shape a naive rewrite mangles: `"O'Brien"` (invalid SQL), `"say ""hi"""` (silent corruption), `"back\\slash"` (double-escaping), `LIKE "%50\% off%"` (escaped wildcard), `!= ""`, `:named` placeholder coexistence, multi-statement, and the real production call site `LIKE "upm%"`. Plus: a comment carrying quoted **prose** alongside a real literal, where only the literal is rewritten; a reserved word used as an identifier; exact byte-for-byte layout preservation; quotes inside single-quoted strings and backticked identifiers; unterminated input; and a regression test that index DDL carrying a double-quoted JSON path is left alone.

- `scanMySqlSql()` is exported from `lib/sql-transform/MySQLSqlTransformer.js` for callers that need the same lexical view of a MySQL statement (quoted runs, comments, everything else).
