...
- 2026-09-22 (unreleased)
  ...
  - (fix) **A failed `set()` auto-save no longer crashes the process.** `set()` saves on a timer and nothing awaits that save, so a failure was an unhandled rejection, which ends Node 15+. It now goes to a new, overridable `onAutoSaveError(error)` instance hook, which logs by default (and a hook that throws is logged too). Note: on a plain `DatabaseObject` that save still fails, because `update()` calls `patch(undefined)`; it now logs instead of crashing. Models whose `patch` override accepts `undefined` (Rubber's does) are unaffected.
