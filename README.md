...
1. The built-in `before_insert_<table>_set_id` trigger for a `uuidKey` table
   (created automatically -- authors never write it). For a table name over
   43 characters that name would pass MySQL's 64-character limit, so it is
   fitted: the first 55 characters, `_`, and 8 hex characters of its SHA-1
   (`idTriggerName` in `lib/identifiers.js`). A name that fits is unchanged.
...
- **Storage:** VARCHAR(36) on Postgres (whose `t.uuidKey` is a native UUID that
  ...
  `varchar(36)` under `stringLinkedIds`. On MySQL, set `linkColumnCollation: true`
  as well so they compare exactly (`utf8mb4_bin`) like the ids they hold; without
  it they take the server default, which ignores case and accents.
...
module.exports = {
  development: {
    ...
    idleTimeout: 60,    // ...and reap after a minute instead of the 600s default
  },
};
```

`minimumIdle`, `acquireTimeout` and `idleTimeout` are only forwarded to the
driver when you set them, so leaving them out preserves yass-orm's own defaults
exactly (`idleTimeout` falls back to 600s inside the dialect; the other two fall
back to the driver's). (The first-connect-probe watchdog described above is a
**separate**, always-on yass-orm default — it does not forward anything to the
driver, so it does not change what the driver sees when these options are
unset.)
...
| `ensureLockTable(db)` | runs | creates `yass_locks` if missing, outside any transaction | same |
...
- `yass_locks` must exist before a transaction that may lock is open: on
  MySQL a table created after a transaction's first read is refused to it
  (*Table definition has changed, please retry transaction*). Schema sync
  (`syncSchemaToDb`, `bin/schema-sync`) creates it; a service that doesn't
...
---
- 2026-09-25 (unreleased)
  - (fix) **MySQL: the first `lockKey` on a fresh database failed** with *Table definition has changed, please retry transaction* (`ER_TABLE_DEF_CHANGED`; found by Tessera's knowledge tests). `lockKey` created its table, `yass_locks`, on first use, from another pool connection, while the caller's transaction was open; InnoDB refuses a transaction any table created after its snapshot (its first read), so once the table existed everything passed, but a fresh production database would have failed its first lock. Now the table is made before any transaction that may lock: **schema sync creates it** (`syncSchemaToDb`, once per process, before the table's own DDL; a failure is reported in that table's `errors`), and new **`sqlHelpers.ensureLockTable(db)`** does it for a service that doesn't sync. Both read the catalog first (`tableExistsHereSql` on the dialects: `information_schema.tables` in `DATABASE()` / `current_schema()`) and run `CREATE TABLE IF NOT EXISTS` only when it is missing, so an existing table sees no DDL (before, every process ran the CREATE on its first lock, which on MySQL waits behind any open transaction using the table); on a create error they look again, so several processes starting at once on a fresh database all succeed (on Postgres, the losing concurrent CREATE can fail on the catalog's unique index). `lockKey` calls `ensureLockTable` on the root handle, so with neither it still creates the table outside the transaction, never inside it (on Postgres a table made inside the caller's transaction would go with its rollback). **`ER_TABLE_DEF_CHANGED` (errno 1412) is now a retryable transaction error**, so `transaction(fn, { maxRetries })` recovers that case (and a transaction that meets a table schema sync just changed). Postgres was not affected: its catalog lookups see a table committed after the transaction's snapshot. **Behavior change for projects that don't use `lockKey`** (Rubber): schema sync now creates the empty one-column table `yass_locks` (one catalog query per sync run once it exists); nothing else reads or writes it. SQLite: unchanged (no row locks, no table).
  - (test) `test/sql-helpers.lock-table.test.js` (live, MySQL and in `npm run test:postgres`): with `yass_locks` dropped, a fresh node process (`test/helpers/lockFreshProcess.js`) that syncs a table, reads in a transaction and then locks succeeds; so does one that calls `ensureLockTable`, six processes doing either at once, one that does neither (with `maxRetries: 1`), and one whose locking transaction rolls back (the table stays); `ensureLockTable` on an existing table runs one catalog read and no DDL, then nothing, and runs nothing on the transaction. `test/transactions.unit.test.js`: `ER_TABLE_DEF_CHANGED` / 1412 is retryable. Red first: on MySQL the sync, several-processes and neither cases failed with `ER_TABLE_DEF_CHANGED`; on Postgres only for the missing `ensureLockTable`.
- 2026-09-24 (unreleased, **major: 3.0**)
  - (fix) **`set()`'s auto-save saves** (bug 14 of the modernization plan). `set()` assigns at once and schedules one save 300ms later (`PATCH_DEFER_DELAY`), but that save was `this.update()` with no arguments, so `patch(undefined)`: on a plain model it threw (logged by `onAutoSaveError` since step 2); a `patch` override that accepts `undefined` (Rubber's) patched only `updatedAt` and read the row back, silently reverting the field `set()` changed. Now each schema field `set()` assigns is an unsaved edit (new `lib/model/auto-save.js`, a non-enumerable symbol on the instance) until that save, which is `this.update(changes)` with the values `set()` was given (the last one per field), so an override gets them through `this.patch(data)` as for any `update(data)`. Until the save starts: a read of the row onto the instance (a `get()` freshening the cached instance, another write's read-back, a transaction publishing its copy at commit, a cache override copying fields) keeps the new value on top rather than undoing it; a `patch()` of the same field on this instance is the later write, so once it has written it wins and that field is not saved again (a failed `patch()` keeps the edit; the save's own `patch()`, however an override reaches it, runs in an `AsyncLocalStorage` scope and doesn't count); `patchIf()` supersedes only the fields it really wrote (it compares with the instance, so `patchIf({ name })` of the unsaved value writes nothing and the edit is still saved). A `set()` while a save runs is saved next, and the running save's read-back doesn't undo it. `reallyDelete()` cancels the pending save. A field not in the schema is only assigned (no save). **This is a behavior change: `set()` now writes.** Rubber calls `set()` on no model instance (checked 2026-09-24: every `.set(` in its source is a `Map`, `URLSearchParams`, Redis/KV, PostHog, THREE.js or similar receiver), so it changes nothing there. Not handled, so use `patch()` where they matter: (1) the save runs outside any transaction, so a `set()` on an instance read with `{ tx }` is saved even if the transaction rolls back (inside a transaction use `patch(data, { tx })`); (2) a write of the same field that doesn't go through this instance's `patch()` before the save (`findOrCreate`, another instance of the row, another process, a transaction's own copy) is overwritten by the `set()` value; (3) once the save has started, a `patch()` of the same field races it like any two concurrent patches.
  - (test) `test/obj.characterize.find-search.test.js` (live, MySQL and in `npm run test:postgres`): the save is `update({ name, points })`; the known-bug test is on (a plain model saves); a `get()`, a `patch()` of another field and a transaction's write at commit don't undo an unsaved `set()`; a `patch()` of the same field wins, and one that fails doesn't drop the edit; `patchIf()` of the unsaved value still saves it, and one that writes the field wins; a `set()` during the save is saved next; `reallyDelete()` cancels the save; a field not in the schema is not saved. Tests wait on the save by polling (`eventually` in `test/helpers/characterize.js`), not a fixed delay. `test/obj.characterize.rubber-subclass.test.js`: the override gets `patch(['name'], undefined)`, its known-bug test is on, and with Rubber's field-copying `setCachedId` a `get()` or a `patch()` of another field doesn't undo the `set()`. `test/obj.set-autosave.test.js`: several `set()`s save once with the last value. Red first; each place that puts the edits back has a test that fails without it.
  - (fix) **MySQL: a `t.uuidKey` table named with more than 43 characters failed schema sync** with *Identifier name 'before_insert_..._set_id' is too long*. The built-in id trigger is named `before_insert_<table>_set_id` (21 characters around the table name), and nothing fitted it to MySQL's 64-character limit. Now one helper, `idTriggerName` (new `lib/identifiers.js`), names it for both the trigger sync creates and the orphan dropped when a table flips to `t.idKey`; the reconciler matches the trigger by that name (it is exempt from the drop pass, and a declared trigger chains `FOLLOWS` it). A name that fits is **unchanged**, so an existing database sees no DDL. A longer one is fitted the way long Postgres index names already were (`fitIdentifierToLimit`, moved to `lib/identifiers.js` and still exported from `lib/sync-to-db.js`): the first 55 characters, `_`, and the first 8 hex characters of the full name's SHA-1, so it is stable across syncs and distinct for two tables sharing a long prefix. Postgres was not affected: it makes no per-table trigger or function (`supportsUuidIdTrigger` and `supportsDeclaredTriggers` are false), and its index names were fitted already. The limits are now named constants, `MYSQL_MAX_IDENTIFIER_LENGTH` (64) and `POSTGRES_MAX_IDENTIFIER_LENGTH` (63), exported with their dialects.
  - (test) `test/identifiers.unit.test.js` (the names unchanged up to a 43-character table, fitted from 44, deterministic and distinct; the dialect constants) and `test/schemaSync.longTableName.test.js` (live, MySQL and in `npm run test:postgres`: a `t.uuidKey` table named with the dialect's full limit syncs, re-syncs to no DDL and takes a model's create and read; on MySQL the fitted trigger sets the id of a raw `INSERT`, a declared `BEFORE INSERT` trigger fires after it, a def with `triggers: {}` keeps it, and a flip to `t.idKey` drops it). Red first on MySQL; the Postgres half passed before the fix, confirming Postgres was unaffected.
...
- 2026-09-23 (unreleased)
  ...
  - (test) `test/transaction.connection-lost.unit.test.js`: a fake dialect loses the connection while leasing, on `BEGIN`, in the callback, after `COMMIT` was sent, and while releasing after it; the first two are retried with the callback run once, the rest surface the error with the callback run once; so does a callback that loses the connection after a transaction in it committed (directly or under a nested `retryIfConnectionLost`). Red first.
  - (fix) `retryIfConnectionLost` rethrows a thrown `null` or `undefined` as is; its connection-lost check (now one `isConnectionLost` helper, shared by the retry and the reconnect-failed paths) used to raise a `TypeError` on it.
---
- 2026-09-23 (unreleased)
  ...
  - (types) `index.d.ts`: every `t.*` field type is typed (`StringField`, `NumberField`, `DateField`, `DateTimeField`, `JsonField`, `LinkedFieldType<T, N>` ...), plus `defineModel`, `applyTableNames`, `DefinedModel<S>`, `ModelFields<S>`, `ModelInstance<S>`, `ModelData<S>`, `ModelInput<S>`, `ModelZodSchema<T>`, `AnyModelClass` and `ModelInstanceMethods` (the instance methods without `DatabaseObject`'s loosely typed data fields; `DatabaseObjectInstanceMethods` extends it). `DatabaseObject`'s statics constrain `this` to `AnyModelClass` rather than `typeof DatabaseObject`, and links, the registry and `checkLinks` take any `AnyModelClass`, so a defined model (whose `id` may be a number) fits: looser only. `LinkedFieldType`'s parameter is now the link target (a model class still works, and `__linkedModel` is unchanged), so a registered-name link's model is looked up only when read and names may form cycles. A bare `LinkedFieldType` accepts every `t.linked(...)`; the one annotation that no longer compiles is `LinkedFieldType<typeof User>` on a lazy reference (`t.linked(() => User)`): write `LinkedFieldType` or `LinkedFieldType<() => typeof User>`.
  ...
  - (test) The three `known bug:` tests are on; new `test/dbh.create-read-back.test.js` (live, MySQL and in `npm run test:postgres`) and a schema-sync-after-close test; each verified red first. **The second test database is configurable**: `schema2` in the test config (default `yass_test2`), so parallel runs can each have their own; `test/test.js` creates its `yass_test3` table there if missing.
...
- 2026-09-22 (unreleased)
  ...
  - (fix) **Models made straight from `loadDefinition()` no longer share one instance-cache bucket.** The cache was keyed by class name, and every such model is an anonymous class named `ModelClass`, so two of them handed out each other's cached instances for the same id (`B.get(1, { allowCached: true })` could return an `A`), and `clearCache()` on one cleared them all. The bucket is now `<class name>:<table>`: separate per model, still shared by two copies of the same class (one module loaded twice), which is what the `globalThis` cache is for. The keys inside `globalThis.__YASS_ORM_OBJECT_CACHE__` change accordingly. Models that override `getCachedId`/`setCachedId` (as Rubber does) are unaffected.
  - (fix) **Reads and writes inside a transaction no longer go into the shared instance cache.** `get(id, { tx })`, `create(..., { tx })`, `patch(..., { tx })` and friends cached the transaction's rows (and changed an already-cached instance in place) before commit, so after a rollback `get(id, { allowCached: true })` served data that was never committed. They now live in a per-transaction cache (`lib/txInstanceCache.js`) that the transaction reads first, so it still sees its own writes and link cycles still resolve. On commit they are published through the model's own `setCachedId`: what the transaction wrote always, anything it only read (or inserted) only when the shared cache has nothing for that id, so a snapshot taken early in a long transaction cannot overwrite a newer instance someone else cached meanwhile; links resolved inside the transaction are then repointed at the shared instances, so the cache keeps one object per row. On rollback nothing is published, and an id written through an instance from outside the transaction is evicted through `removeCachedId`. A rolled-back savepoint drops the transaction's instances at once (the rest of the transaction reads afresh), and a later commit evicts instead of publishing. Consequence: inside a transaction, `get(id, { tx })` returns a different object from the one already in the shared cache (the shared one is freshened at commit), and a model's `getCachedId`/`setCachedId`/`removeCachedId` overrides are no longer called during the transaction, only when it ends. Only handles from `dbh.transaction()` are scoped; any other `tx` value behaves as before. `remove({ tx })` likewise leaves the shared cache alone until the transaction ends. `lib/transactions.js` exports `onTransactionEnd(tx, { commit, rollback, savepointRollback })` and `transactionLocal(tx, key, factory)` (per-transaction state), which this uses.
  - (fix) **Link resolution is cached per linking folder.** The resolved-path cache was keyed by the relative link name only, so two models in different folders linking to the same name (`t.linked('target')`) both got whichever file was resolved first. It is now keyed by the linking model's `basePath()` and the name.
  - (fix) **An already-inflated linked instance is kept as it is.** `_resolvedLinkedModel` meant to skip `get()` when handed an instance of the linked model, but its guard (`modelId.id === modelId`) could never be true, so it always went through `get()` (a cache lookup by the instance's `toString()`, or a query). It now returns the instance.
  - (fix) **A failed `set()` auto-save no longer crashes the process.** `set()` saves on a timer and nothing awaits that save, so a failure was an unhandled rejection, which ends Node 15+. It now goes to a new, overridable `onAutoSaveError(error)` instance hook, which logs by default (and a hook that throws is logged too). Note: on a plain `DatabaseObject` that save still fails, because `update()` calls `patch(undefined)`; it now logs instead of crashing. A `patch` override that accepts `undefined` (Rubber's does) does not throw, but the save still does not happen: it patches only `updatedAt`, then the row read back overwrites the field `set()` changed (see the characterization entry below).
  - (test) `test/obj.cache-scope.test.js` (live MySQL: the bucket per model, and ten transaction cases incl. rollback, commit, a read-only snapshot at commit, savepoints, a self-link cycle and a Rubber-style cache override), `test/obj.link-resolution.test.js` (two link folders; an instance link skips `get()`), `test/obj.set-autosave.test.js`; each verified red first.
  - (test) **Characterization tests for the model layer** (step 3 of the modernization plan; tests only, no library change). `test/obj.characterize.*.test.js` pin how yass behaves today so that `obj.js` can be split safely:
    - `hydration`: `inflateValues`/`deflateValues` for each type (`parseFloat` even for `t.int`; a boolean is true only for a value `== '1'`; a datetime string is read as UTC; invalid JSON is kept as the raw string, with a warning), and `inflate()` handing out one freshened instance per id;
    - `links`: `t.linked` by name, relative path and absolute path (the shape `withRelativeModelLinks` produces, via a CommonJS copy of it in `test/helpers/`), `{ inverse }` accepted and ignored, links loaded recursively with one instance per row, a missing row inflating to `null`, a missing model file failing at first read, and `jsonify()` with `includeLinked` / `excludeLinked`;
    - `cache`: the instance cache outside and inside transactions (`get` with and without `allowCached` and `tx`, create/patch/remove/reallyDelete, commit, rollback, savepoints, a `tx` that is not a transaction handle);
    - `find-search`: `findOrCreate` (with its hook order), `search`, `searchOne`, `fromSql`, `withDbh`, `queryCallback`, `set()` and `update()`;
    - `rubber-subclass`: a model built like Rubber's `_shared-base.js` (`test/fixtures/characterize/rubber-base.js`), overriding `get`, `getCachedId(id, span)`, `setCachedId`, `removeCachedId`, `afterCreateHook` / `afterChangeHook(txOptions)`, `patch` and `findOrCreate`. It asserts the order yass calls them in (e.g. `get` > `inflate` > `getCachedId(id, span)` > `setCachedId` twice on a miss), that every call reaches the override through `this`, and that `{ tx }` is passed on;
    - `contract`: the exports, the deep imports Rubber uses (`yass-orm/lib/config.js`, `lib/dbh`; ESM default and named imports, from a consumer's `node_modules`), the bin paths, and the `globalThis` keys (yass adopts values already there; `__YASS_ORM_MODEL_PATH_INDEX__`, `__YASS_ORM_PATH_RESOLVER__`, `__YASS_DEF_PATH_MAP__` and `__YASS_ORM_DEFINITION_INDEX__` are honored).

    The database-backed files run on MySQL in `npm test` and on Postgres in `npm run test:postgres` (added there).
  - (test) **Bugs found and not fixed**, each a skipped test named `known bug: …`, ready to unskip once fixed:
    - `jsonify({ includeLinked: true })` on a row that links to itself never resolves: the cycle guard hands the nested call the outer call's own pending promise;
    - while an `includeLinked` `jsonify()` is pending, any other `jsonify()` on the same instance returns that call's result, whatever options it asked for;
    - MySQL, without `uuidLinkedIds`: `findOrCreate()` on a `t.uuidKey` model reads the new row back with `WHERE id = 0` (`dbh.create()` generates an id only when `uuidLinkedIds` is set). MySQL compares that numerically, so it returns another row whose id starts with a letter or zeros, or none (and throws). Rubber sets `uuidLinkedIds`, so it is not affected;
    - Postgres: `fromSql()` with no arguments sends `WHERE 1`, which Postgres rejects;
    - after `closeAllConnections()`, every model call and `syncSchemaToDb` fail with *pool is closed*: `lib/utils.js` `handle()` and `lib/sync-to-db.js` keep the closed handle. `dbh()` makes a new pool;
    - `set()` auto-save never saves: `update()` calls `patch(undefined)`. On a plain model that throws (known since step 2). A Rubber-style `patch` override doesn't throw, but it patches only `updatedAt`, and then the row read back reverts the field. (Fixed 2026-09-24, bug 14.)
  - (change) **The config is loaded on first use, not at `require`.** `lib/config.js` used to find and load `.yass-orm.js`, print its notices, and throw `Unknown config env for YassORM: <env>` the moment anything required yass, so a process with, say, `NODE_ENV=test` could not even import it. It now exports a proxy that does all of that the first time a property is read, written, listed or inspected. So requiring yass (or deep-importing `yass-orm/lib/config.js`, `lib/dbh`, `lib/sync-to-db`) never throws or prints; `YASS_CONFIG`, `YASS_ENV` and `NODE_ENV` may be set after `require` and before first use; an unknown env throws on first use instead (and a later use tries again). Once loaded it is the same object with the same values, keys and key order as before: `require('yass-orm').config === require('yass-orm/lib/config.js')`, and writes, `delete`, `in`, `Object.keys`, spread, `JSON.stringify` and `console.log` all work on it. `dbh.js`, `sync-to-db.js` and `def-to-schema.js` no longer copy config values into module constants at load; they read them when used (so a test that sets `config.commonFields` now takes effect). The load balancer takes `readBalanceStrategy` from the config when it first needs it; anything that sets its strategy still wins. Note: `.yass-orm.js` itself is also `require`d on first use, so its top-level side effects (such as the common `process.env.NODE_ENV = 'development'` line) now run then, not when yass is required.
  - (test) `test/config.lazy.test.js` (fresh child processes: requiring with an unknown env and no config neither throws nor prints; the error comes on first use; a late `YASS_CONFIG` takes effect in config, `dbh`, `def-to-schema` and schema sync; one export, identical values, keys/spread/JSON/inspect; production and staging; the read-balance strategy; verified red first). `test/obj.cache-scope.test.js`, `test/obj.transaction.test.js` and `test/test.js` no longer hard-code the `test` schema in their raw SQL, so the suite runs against any configured schema.
---
- 2026-09-10 (2.7.0)
  ...
  - (feat) **An invalid `idleTimeout` throws, naming the option.** Must be `undefined` or a positive whole number of seconds. `0` is rejected deliberately: mariadb does `opts.idleTimeout || 1800`, so falsy is indistinguishable from unset and "never reap" is inexpressible. The driver throws for no input at all (`-5`, `1.5`, `'abc'` all pass through), so this cannot be left to it.
  - (fix) Both dialects moved from `config.idleTimeout || 600` to `=== undefined ? 600 : ...`, so a `0` is not silently rewritten one layer below `dbh.js`.
  - (test) `test/dbh.idleTimeout.test.js`: drives `dbh()` with `mariadb.createPool` stubbed (no connection made) and asserts the value reaches **both** the write pool and the RO pool, that the key is **absent** when unset, that `MySQLDialect` still supplies exactly `600`, and that six invalid values each reject. Every forwarding case carries `minimumIdle` as an in-run positive control.
---
- 2026-09-04 (2.6.1)
...
---
- 2026-08-25 (2.5.1)
  - (fix) **`dbh()` leaked an ENTIRE POOL per concurrent caller on a cold cache** (BC-3587). `dbh()` is `async`, with roughly a thousand lines and several `await`s between the `connCache` LOOKUP and the `connCache` WRITE, and no in-flight guard — so N callers asking for the same handle before the first resolved each built their **own** pool and only the last to finish won the cache slot. The other N-1 were **orphaned**: never handed out, never closed by `closeAllConnections()` (it only walks `connCache`), never reaped (the mariadb driver defaults `minimumIdle` to `connectionLimit`, so `idleTimeout` trims nothing). The damage compounds per cold-cache burst — exactly what a test suite does — and server-side connections ramp **monotonically** until `max_connections` is exhausted, after which every acquire fails with errno 45028 / `HY000` "retrieve connection from pool timeout". Distinctive signature, visible in CI: pool `total` climbing `11 → 41 → 101 → 140` while `active` stayed at 0–3. Measured before the fix: 20 concurrent `dbh()` calls produced **20 distinct pools**, three cold-cache rounds leaked **+23 → +107 → +188** connections past teardown; after, 1 pool per round, flat at +1. Reproduce with `node test/manual/pool-leak-probe.js` (exits non-zero if the leak returns). In-flight creations are now memoized in a `pendingConnCache` keyed like `connCache`, dropped on both success and failure so a failed create cannot poison retries; `closeAllConnections()` settles in-flight creations first. `ignoreCachedConnections: true` deliberately opts out of both caches — it exists to hand out an extra throwaway handle, and deduping it would break that contract.
  - (feat) **`minimumIdle` and `acquireTimeout` pool options** (config + per-`dbh()` call), forwarded to the driver only when set. These are the two levers for bounding connection usage: the driver defaults `minimumIdle` to `connectionLimit`, so a pool grows to its limit and holds it forever because `idleTimeout` never has anything above the floor to reap. `minimumIdle: 0` lets a long-lived process or test suite give idle connections back. Leaving both unset preserves current behavior exactly — they are omitted from the driver config rather than passed as `undefined`.

---
- 2026-08-24 (2.5.0)
  - (fix) **`searchOne()`'s SECOND POSITIONAL was a silent no-op for ordering** (BDL-2700). 2.4.0 gave `search()`'s second positional a validated option vocabulary (`limitOne, limit, offset, orderBy, orderDir`); `searchOne` was not extended — it hardcoded `true` into that validated slot and forwarded the caller's object to the THIRD (pool-config) slot, where an ordering bag is meaningless. So `searchOne(fields, { orderBy: 'createdAt', orderDir: 'DESC' })` emitted **no `ORDER BY`**, threw nothing, and returned an arbitrary row — a wrong answer wearing a correct-looking call. A pool config there was inert too, since `search()` returns inside its `limitOne` branch before `promisePoolMap` is reached. **No instrument could catch it:** `PromisePoolMapConfig` carries `[key: string]: any`, so excess-property checking can never fire and the bad call sites type-checked clean. The slot is now PARTITIONED across its three legitimate vocabularies — ordering (`orderBy`/`orderDir`, validated against the model schema by `search()`), pool config (`concurrency, debug, logger, throwErrors, yieldEvery`), and `tx` — and anything else throws, naming the key the caller actually typed.
  - (fix) **`warnOnInvalidDeflateKey` was UNREACHABLE, not merely off.** `deflateValues` is a STATIC method, so inside it `this` IS the class — and it read the hook as `this.constructor.warnOnInvalidDeflateKey`, which walks past the class to `Function` and is always `undefined`. No model could turn the diagnostic on by any documented path. Now reads `this.warnOnInvalidDeflateKey`. The **default is unchanged** (`deflateValues` still ignores non-schema keys, which is intended — the opt-in hook is the evidence of that intent), so this is inert until a model installs one.
  - (breaking, in a slot that measurably had no callers) `searchOne`'s second positional now THROWS on a key outside those three vocabularies, where it previously swallowed it. Also newly rejected: `limitOne` (implied by the method) and `limit`/`offset` (they contradict the single-row return shape — use `search()` for a bounded page). A non-object, non-nullish second argument now throws instead of being ignored. Measured across the rubber monorepo with a paren-balancing, string-aware scanner: **2194 `searchOne` call sites, 4 with a non-empty second argument, 2 of them live** — and both were the bug (`{ sort: {...} }` and `{ orderBy: 'lastStatusAt DESC' }`). **Zero** passed a genuine pool config.
  - (types) New exported `SearchOneOptions`, used by BOTH the instance and static `searchOne` forms in `index.d.ts` and by the emitted per-model interface in `lib/generate-types.js`. It deliberately has **no index signature**, which is what lets the compiler reject a mistyped option at the call site — fixing the runtime alone would have left the types waving the next bad call through, the same one-of-three-layers omission that produced the bug.
...
---
- 2026-08-18 (2.4.1)
  - (fix) **generate-types: description text is now escaped in emitted `.describe()` literals** (BDL-2852). The Zod emitter built its description literal by hand (`description.replace(/'/g, "\\'")`), which escapes apostrophes and nothing else. Any `.description()` containing a raw newline terminated the single-quoted literal mid-string, so the model's own generated `.zod.ts` stopped parsing — measured downstream at **1,189 errors, 100% TS1xxx PARSE errors** (TS1002/TS1005/TS1127/TS1434), zero type errors. Not newline-specific either: a backslash immediately before a quote closed the string just as effectively. Emission now goes through `JSON.stringify` (plus explicit U+2028/U+2029 escaping, which are legal in JSON but are JS LineTerminators) at all four `.describe()` sites. Fixed by ESCAPING, not by restricting the input — multi-line `.description()` text is documentation we want authors to write.
  - (fix) **`.d.ts` JSDoc emission no longer breaks on the same input.** It consumes the same description string; an end-of-comment sequence in the text closed the block comment early, and raw newlines produced unprefixed continuation lines.
  - (note for anyone repinning rubber) This release exists as a distinct version ON PURPOSE. `scripts/update-yass-orm.sh` verifies an install by comparing the installed version against the version the pin resolves to, so a behaviour change landed WITHOUT a version bump makes that verifier vacuous — it prints the same ✔ whether or not the new code arrived.

---
- 2026-08-17 (2.4.0)
  - (feat) **`Model.search(fields, options)` now accepts a real options object** — `{ limitOne, limit, offset, orderBy, orderDir }` — and throws on anything else. The second positional was a bare `limitOne` boolean, so the natural-looking `Model.search(fields, { limit: 20, orderBy: 'createdAt' })` passed a truthy **object** into the boolean slot: the emitted SQL was `limit 1` with **no ORDER BY at all**, and the return value was a **single instance instead of an array**. No error, no warning. Three mutually incompatible dialects (`orderBy`/`orderDir`, `sortBy: ['-col']`, `sort: {col:-1}`) had grown up across callers and **none of them had ever worked**. `limitOne` (boolean, `undefined`, and the omitted form) is unchanged and byte-identical on the old path. `orderBy` is validated against the model's own schema field list and emitted through `escapeId`, so caller text is never interpolated into SQL; `orderDir` accepts only `ASC`/`DESC`; `limit`/`offset` must be non-negative integers. Unknown keys throw, naming the offending key — the previous behaviour was to silently do something else, which is the whole reason this exists. `Model.find()` remains the separate Feathers-style `$limit`/`$skip`/`$sort` surface. Known gap, deliberately out of scope: option keys smuggled into the **fields** object (`search({ user, sortBy: 'label' })`) are dropped silently by `deflateValues`, so the validator never sees them.
  - (fix) **MySQL: double-quoted string literals are normalized to single quotes**, so the same SQL means the same thing under `ANSI_QUOTES`. MySQL was the only dialect with no normalization pass (`transformSql` was a literal identity function), so caller SQL like `... LIKE "upm%"` reached the server byte-for-byte. Under default `sql_mode` that is a string; under `ANSI_QUOTES` it is an **identifier**, so the query succeeds on a primary and dies with `ERROR 1054 unknown column 'upm%'` on an `ANSI_QUOTES` read replica — routing-dependent, intermittent, and with an error pointing at the schema rather than at quoting. New `lib/sql-transform/MySQLSqlTransformer.js` lexes the statement and **decodes out of the double-quote context and re-encodes into the single-quote one** rather than string-replacing across the encoding boundary (a naive escape corrupts `""` with no error). The rewrite **splices the original source** instead of reserializing: comments survive byte-for-byte, and SQL that `node-sql-parser` cannot parse at all (a reserved word as an identifier) is fixed too. A `"` inside a comment, single-quoted string, or backticked identifier is left alone. `--` only starts a comment when whitespace follows, per MySQL — `a--1` is `a - (-1)`, and mis-lexing it would silently delete a predicate. DDL stays excluded for a **text-matching** reason, not a semantic one: `meta->>'valence'` means the same as `meta->>"valence"` but no longer matches the index definition MySQL reports back, so `schemaSync` would drop and recreate the index on every run. Known limitation by design: SQL authored *for* `ANSI_QUOTES`, where `"col"` is meant as an identifier, is indistinguishable from a string literal here — do not feed this ANSI-authored SQL.
  - (test) 38 tests in `lib/sql-transform/test/MySQLSqlTransformer.test.js`, including canaries for every shape a naive rewrite mangles: `"O'Brien"` (invalid SQL), `"say ""hi"""` (silent corruption), `"back\\slash"` (double-escaping), `LIKE "%50\% off%"` (escaped wildcard), `!= ""`, `:named` placeholder coexistence, multi-statement, and the real production call site `LIKE "upm%"`. Also exports `scanMySqlSql()` for callers needing the same lexical view of a MySQL statement.
