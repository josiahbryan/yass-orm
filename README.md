...
- 2026-09-22 (unreleased)
  ...
  - (test) **Bugs found and not fixed**, each a skipped test named `known bug: …`, ready to unskip once fixed:
    ...
    - `set()` auto-save never saves: `update()` calls `patch(undefined)`. On a plain model that throws (known since step 2). A Rubber-style `patch` override doesn't throw, but it patches only `updatedAt`, and then the row read back reverts the field.
