...
const SqlSchemaDefinitionTypes = {
	...
	// 'object' is an type in our client that stores JS objects as JSON.
	// server-side we store as longtext
	// object: { type: 'longtext' },
	object: (options) => {
		const { expand, schema, template = {} } = options || {};
		return (field, model, expandedModel) => {
			const extension = {};
			if (expand || schema) {
				const x = expand || schema;
	...
	shape: (template = {}) => {
		return SqlSchemaDefinitionTypes.object({ template });
	},
	linked: (type, { cascadeRemove = false } = {}) => {
