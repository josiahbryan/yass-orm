...
const SqlSchemaDefinitionTypes = {
	...
	// Options:
	//   - expand/schema: Define sub-fields (legacy wrappers)
	//   - Direct schema fields: t.object({ field1: t.string, ... })
	//   - noExpand: true - Keep schema for TypeScript/Zod but DON'T create SQL columns
	//     Use this when you want type safety but just want to store JSON in one column
	object: (options) => {
		return (field, model, expandedModel) => {
			const extension = {};

			// Check for noExpand option - if true, we capture schema for types but
			// don't create separate SQL columns (just store as JSON in one longtext column)
			const noExpand = options && options.noExpand === true;

			// Support three formats:
			// 1. t.object({ expand: {...} }) - legacy expand wrapper
			// 2. t.object({ schema: {...} }) - legacy schema wrapper
			// 3. t.object({ field1: t.string, ... }) - direct schema fields (preferred)
			// 4. t.object({ schema: {...}, noExpand: true }) - schema for types only, no SQL columns
			let schemaObj = null;
			if (options) {
				if (options.expand || options.schema) {
					// Legacy wrapped format or explicit schema with noExpand
					...
					if (optionKeys.length > 0) {
						...
						if (hasTypeFields) {
							// Filter out noExpand when creating schemaObj
							schemaObj = {};
