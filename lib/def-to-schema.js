...
const SqlSchemaDefinitionTypes = {
	id:      { type: 'primaryKey', nativeType: Number  },
...
function expandType(value, field, model, jsonModel) {
	...
	if(typeof(value) === 'function' && value.__typeConverter)
		value = value();
...
function toSchema(definition) {
	...
	if(!model.schema)
		throw new Error('definition.schema does not exist');
	...
	model.schema = Object.assign({}, model.schema, isDeletedSchema(SqlSchemaDefinitionContext));

	if(model.includeCommonFields)
		model.schema = Object.assign({}, model.schema, commonFields(SqlSchemaDefinitionContext.types));

	const fields = Object.keys(model.schema);
	if(!fields.length)
		throw new Error('definition.schema is empty');
	...
	jsonModel.fieldMap = Object.assign({}, fieldMap, {
		id: Object.assign(
			expandType(SqlSchemaDefinitionTypes.id, 'id', model, jsonModel),
			{ field: 'id' }
		)
	});
...
function convertDefinition(modelDefinition) {
	const schema = toSchema(modelDefinition.default ? modelDefinition.default : modelDefinition);
	return schema;
}
