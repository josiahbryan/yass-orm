...
const { validateTriggerSpec } = require('./sync-triggers');
const { isDefinedModel } = require('./model/registry');

const CLIENT_ONLY_FIELD = 'CLIENT_ONLY_FIELD';

// Field options that share their name with the chainable method that sets them.
const METHOD_OPTION_KEYS = ['exact', 'precision'];

/**
 * `precision` for t.datetime: whole digits of fractional seconds, 0-6 (the
 * range MySQL's DATETIME(fsp) accepts). Throws at definition time otherwise.
 * @param {*} n
 * @returns {number}
 */
function checkPrecision(n) {
	if (!Number.isInteger(n) || n < 0 || n > 6) {
		throw new Error(
			`t.datetime precision must be a whole number from 0 to 6, got ${JSON.stringify(
				n,
			)}`,
		);
	}
...
/**
 * Resolve the collation for a char(36) UUID-style link/uuid column, or a
 * varchar(36) link column under `stringLinkedIds` (bug 17: those hold
 * `t.stringKey` ids, which are utf8mb4_bin too).
 *
 * OPT-IN, default OFF (zero behavior change): controlled by `linkColumnCollation`
 * in the yass-orm config.
 *   - unset / falsy  -> returns undefined (no collation emitted; legacy behavior).
 *   - `true`         -> CANONICAL_UUID_COLLATION (utf8mb4_bin), identical to the
 *                       uuid PRIMARY KEY collation BY CONSTRUCTION (same constant).
 *   - a string       -> that explicit collation (advanced override).
 *
 * Only these 36-char id-holding columns can mismatch the id they join to, so
 * this never touches int-mode links (uuidLinkedIds:false), array links or
 * other columns.
 *
 * Reads `config` live (not a load-time snapshot) so tests can toggle the flag.
 *
 * @param {string} dataType - the resolved SQL column type
 * @returns {string|undefined}
 */
function resolveLinkColumnCollation(dataType) {
	if (dataType !== 'char(36)' && dataType !== 'varchar(36)') {
		return undefined;
	}
	const opt = config.linkColumnCollation;
	if (!opt) {
		return undefined;
	}
	return opt === true ? CANONICAL_UUID_COLLATION : String(opt);
}
...
function getChainableTypeData(obj) {
	...
	for (const key of Object.keys(obj)) {
		if (
			// A lazy-reference link, t.linked(() => Model), is a function but data
			(typeof obj[key] !== 'function' || key === 'linkedModel') &&
	...
	Object.assign(data, obj.__methodOptions);
	// When 'default' was overwritten by the .default() method, use __defaultValue for data
	if (
		typeof obj.default === 'function' &&
		Object.prototype.hasOwnProperty.call(obj, '__defaultValue')
	) {
		data.default = obj.__defaultValue;
	}
	return data;
}
...
function createChainableType(baseType, optionsHandler = null) {
	...
	// Options named like the method that sets them (`precision: 3` from
	// `.precision(3)`, `exact: true` from `.exact()`): the method takes the key
	// on the function, so the value is kept here and put back when the type is
	// read (getChainableTypeData, stripChainableMethods).
	chainable.__methodOptions = {};
	METHOD_OPTION_KEYS.forEach((key) => {
		if (baseType[key] !== undefined && typeof baseType[key] !== 'function') {
			chainable.__methodOptions[key] = baseType[key];
		}
	});
	...
	// Preserve initial default value before the .default() method overwrites it
	// This ensures types like t.bool (which have default: 0) retain their default
	// when getChainableTypeData() is called later for SQL generation
	// Bug fix: Without this, t.bool's default: 0 was lost when .default() method
	// was attached, causing isDeleted columns to be created without DEFAULT 0
	if (
		Object.prototype.hasOwnProperty.call(baseType, 'default') &&
		typeof baseType.default !== 'function' &&
		!Object.prototype.hasOwnProperty.call(baseType, '__defaultValue')
	) {
		chainable.__defaultValue = baseType.default;
	}
	 ...
	 * Always add the method - it will override any existing default data property.
	 * Only skip if 'default' is already a function (to avoid conflicts).
	 */
	// Attach .default() unless baseType.default is already a function (so we don't overwrite an existing method).
	if (
		!Object.prototype.hasOwnProperty.call(baseType, 'default') ||
		typeof baseType.default !== 'function'
	) {
		chainable.default = function setDefaultValue(value) {
			const currentData = getChainableTypeData(this);
			return createChainableType(
				{ ...currentData, default: value, null: 0, __defaultValue: value },
				this.__optionsHandler,
			);
		};
	}
	...
	if (isStringType) {
		...
		/**
		 * Compare case- and accent-exactly (for emails, tokens, identifiers).
		 * MySQL/MariaDB: COLLATE utf8mb4_bin (their default collation calls
		 * `jose` and `JOSÉ` equal). Postgres and SQLite already compare exactly,
		 * so there it changes nothing. Same as `t.string({ exact: true })`.
		 */
		chainable.exact = function setExact() {
			const currentData = getChainableTypeData(this);
			return createChainableType(
				{ ...currentData, exact: true },
				this.__optionsHandler,
			);
		};
	}

	if (baseType.type === 'datetime') {
		/**
		 * Keep `n` digits of fractional seconds (0-6). MySQL/MariaDB: DATETIME(n)
		 * (a plain DATETIME drops them). Postgres TIMESTAMPTZ always keeps
		 * microseconds, so there it changes nothing. Same as
		 * `t.datetime({ precision: n })`.
		 * @param {number} n - Fractional-second digits, 0-6
		 */
		chainable.precision = function setPrecision(n) {
			const currentData = getChainableTypeData(this);
			return createChainableType(
				{ ...currentData, precision: checkPrecision(n) },
				this.__optionsHandler,
			);
		};
	}
	...
	if (typeObj.__isChainableType || typeof typeObj === 'function') {
		...
		Object.keys(typeObj).forEach((key) => {
			...
			if (isChainableMethod(typeObj[key], key)) {
				// When 'default' was overwritten by the .default() method, expose __defaultValue as default
				if (
					key === 'default' &&
					Object.prototype.hasOwnProperty.call(typeObj, '__defaultValue')
				) {
					result.default = typeObj.__defaultValue;
				}
				return;
			}
			result[key] = typeObj[key];
		});
		return Object.assign(result, typeObj.__methodOptions);
	}
...
const SqlSchemaDefinitionTypes = {
	...
	// An app-generated STRING primary key (see lib/objectId.js). It is a uuidKey
	// for every create/sync path; `stringId` only changes the storage type where
	// uuidKey is not already a string column (Postgres' native UUID).
	stringKey: createChainableType({
		type: 'uuidKey',
		nativeType: String,
		stringId: true,
	}),
	...
	// Large integers - stored in native BIGINT-compatible columns, exposed as strings
	// because JavaScript numbers cannot safely represent integers > 2^53-1.
	bigint: createChainableType({ type: 'bigint', nativeType: String }),

	// UUID field (not primary key) - char(36) for UUID format.
	// Getter (not a static value) so the opt-in linkColumnCollation is resolved
	// live off config: OFF (default) emits no collation (legacy behavior); ON gives
	// it the canonical utf8mb4_bin to stay sargable against uuid PKs it may join to.
	get uuid() {
		const collation = resolveLinkColumnCollation('char(36)');
		return createChainableType({
			type: 'char(36)',
			nativeType: String,
			...(collation ? { collation } : {}),
	...
	array: function array(itemType) {
		...
		if (itemType) {
			...
			if (itemTypeValue === 'bigint') {
				arrayItemType = 'bigint';
			} else if (itemTypeValue === 'varchar' || itemTypeValue === 'longtext') {
	...
	linked: (type, { array = false } = {}) => {
		...
		// (read live off config so tests can toggle it; config is a load-time singleton in prod)
		let dataType = config.uuidLinkedIds ? 'char(36)' : 'int';

		// stringLinkedIds: links point at `t.stringKey` ids (prefixed strings), so
		// they are plain strings everywhere -- not CHAR(36), which Postgres would
		// otherwise store as a native UUID to join `t.uuidKey` keys.
		if (config.stringLinkedIds) {
			dataType = 'varchar(36)';
		}
		...
		// Opt-in (default OFF): give char(36)/varchar(36) link columns the SAME
		// collation as the id they hold, so joins stay sargable and exact. This was
		// the historical performance TODO here.
		const collation = resolveLinkColumnCollation(dataType);
	...
	date: createChainableType(
		...
		(attrOptions) => {
			...
			if (attrOptions && attrOptions.defaultValue) {
				...
				baseType.__defaultValue = attrOptions.defaultValue;
			}
			return createChainableType(baseType);
		},
	),
	...
	datetime: createChainableType(
		...
		// Options handler: t.datetime({ defaultValue: '...', precision: 3 })
		(attrOptions) => {
			const baseType = {
				type: 'datetime',
				nativeType: Date,
			...
			if (attrOptions && attrOptions.precision !== undefined) {
				baseType.precision = checkPrecision(attrOptions.precision);
			}
			return createChainableType(baseType);
		},
	),
...
function toSchema(definition) {
	...
	const {
		...
		includeCommonFields,
		indexes,
		// `triggers` is destructured here (rather than falling through
		// `passThruProps`) so it lands on `options.triggers` next to
		// `options.indexes` -- one place for the DDL reconciler to read from,
		// no duplicate copy leaking onto the top-level schema. See tests in
		// lib/def-to-schema.triggers.test.js.
		triggers,
		table,
		options,
		...passThruProps
	...
	if (includeCommonFields)
		schema = Object.assign(
			{},
			schema,
			config.commonFields(SqlSchemaDefinitionContext.types),
		);
	...
	// TRIGGERS: two acceptable spellings (top-level and options.triggers),
	// merged into options.triggers. If both spellings appear the top-level
	// value WINS -- consistent with how a sibling `indexes` at the top level
	// would land in the same place, and less surprising than a deep merge
	// that could silently combine two authors' work.
	const declaredTriggers = triggers || (options && options.triggers);
	if (declaredTriggers !== undefined) {
		// The `hasTriggersDeclared` sentinel is what gates the drop-undeclared
		// pass in sync-to-db.js. It must be true even when the block is empty
		// -- "declared triggers: none" is a valid, opinionated statement, and
		// it is what makes a rename converge (old name dropped after the new
		// one is created). See the plan's "opt-in authority per table" section.
		jsonModel.options.hasTriggersDeclared = true;
		// Validate each spec at convert time so a typo fails LOUDLY here
		// rather than as a silent no-op that surfaces one deploy later as a
		// missing trigger. Same voice as the multi-valued index validation
		// upstream in lib/sync-to-db.js.
		Object.entries(declaredTriggers).forEach(([name, spec]) => {
			validateTriggerSpec(name, spec);
		});
		jsonModel.options.triggers = declaredTriggers;
	}
...
function convertDefinition(modelDefinition) {
	const definition = modelDefinition.default
		? modelDefinition.default
		: modelDefinition;
	// A defineModel() model converts its definition (so a file whose default
	// export is one works with schema-sync and generate-types).
	return toSchema(
		isDefinedModel(definition) ? definition.definition : definition,
	);
}

module.exports = { convertDefinition, resolveLinkColumnCollation };
