...
async function createDbh(options = { ignoreCachedConnections: false }) {
	...
	// throws for ANY input. This is the single choke point covering both the
	// .yass-orm.js config path and per-call dbh({ idleTimeout }).
	if (
		...
		throw new Error(
			`[yass-orm] Invalid \`idleTimeout\`: ${JSON.stringify(idleTimeout)} ` +
				...
				`expressible -- pass a large finite value (e.g. 86400) instead.`,
		);
	}
