...
const QueryTiming = {
	...
	queries: {},
	analyze() {
		console.log(`[dbh.QueryTiming.analyze] ${Object.values(QueryTiming.queries).length} unique queries recorded:`)
		return Object.values(QueryTiming.queries).map(data => {
			if(!data.length) {
...
// let idField; { table, idField } = parseIdField(table);
	...
	conn.search = function(table, fields={}, limitOne=false) {
		const prep = 'select * from ' + autoFixTable(table),
			...
			list = keys.map(x => mariasql.escape(x)+'=:'+x),
