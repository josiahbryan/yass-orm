...
async function dbh(options = { ignoreCachedConnections: false }) {
	...
	// Using createPool instead of createConnection to enable connection lifecycle management
	const conn = await mariadb.createPool(poolConfig);
	...
	if (activeRoNodes && activeRoNodes.length) {
		readConns = await promiseMap(
			...
			async ({ host, user, password, ssl, port, loadBalancerOptions = {} }) => {
				...
				// https://www.npmjs.com/package/mariadb
				// Using createPool for read-only connections too
				const roConn = await mariadb.createPool(roConnConfig);
				...
				roConn.clone = async () => mariadb.createPool(roConnConfig);
