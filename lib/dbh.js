...
async function dbh(options = { ignoreCachedConnections: false }) {
	...
	const key = connCacheKey({
		dialectName:
			options.dialect === undefined ? defaults.dialect : options.dialect,
		user: options.user === undefined ? defaults.user : options.user,
		host: options.host === undefined ? defaults.host : options.host,
		db: options.db === undefined ? defaults.schema : options.db,
		port: options.port === undefined ? defaults.port : options.port,
		filename:
			options.filename === undefined ? defaults.filename : options.filename,
	});
