 ...
 */
function autoFixTable(table, dbh) {
	if (!dbh) {
	...
	return dbh.escapeId(table.replace(/-/g,'_'));
}
 ...
 */
function sqlEscape(value, dbh) {
	if (!dbh) {
		throw new Error(`Must pass current handle as 2nd arg`)
	}
	return dbh.escape(value);
}
...
async function dbh(options) {
	...
	await conn.query(`use ${conn.escapeId(config.db)}`);
...
==============================
		...
		if (QueryTiming.enabled) {
			const trace = new Error().stack;
			(QueryTiming.queries[sql] || (QueryTiming.queries[sql] = []))
				.push({ sql, queryTime, params, trace })
		}
	...
	conn.create = function(tableAndIdField, fields) {
		...
		return this.pquery(sql, fields).then(result => {
			...
			const select = 'select * from ' + autoFixTable(table, this) + ' where ' + idField + '=:id';
	...
	conn.get = async function(tableAndIdField, id) {
		let { table, idField } = parseIdField(tableAndIdField);
		return this.pquery('select * from ' + autoFixTable(table, this) + ' where ' + idField + '=:id', { id }).then(rows => {
	...
	conn.destroy = async function(tableAndIdField, id) {
		let { table, idField } = parseIdField(tableAndIdField);
		return this.pquery('delete from ' + autoFixTable(table, this) + ' where ' + idField + '=:id', { id });
	}
...
class ProxyServiceInstance {
	...
	async find(params) {
		console.warn("[mariadb.util.ProxyServiceInstance("+this.tableAndIdField+").find] We don't emulate the FeathersJS find() service method exactly (only honor $limit===1, and params are assumed to be straight field=value pairs, for example)...use at your own risk!");
