...
==============================
	...
	conn.search = function (
		...
		return this.roQuery(sql, fields, { silenceErrors }).then((rows) => {
			if (limitOne) {
	...
	conn.create = function (
		tableAndIdField,
		fields,
		{ allowBlankIdOnCreate, idGenerator = uuid, silenceErrors = false } = {},
	) {
		...
		return this.pquery(sql, fields, { silenceErrors })
			.then((result) => {
				...
				return this.pquery(select, selectParams, { silenceErrors });
			})
			.then((rows) => {
	...
	conn.findOrCreate = async function (
		...
		patchIf = {},
		patchIfFalsey = {},
		{ allowBlankIdOnCreate, idGenerator, silenceErrors = false } = {},
	) {
		let { table /* , idField */ } = parseIdField(tableAndIdField);
		conn.findOrCreate.lastAction = null;
		return this.search(table, fields, true, { silenceErrors })
			.then((ref) => {
				if (!ref) {
