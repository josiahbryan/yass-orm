...
const {
	...
	// Floor of connections the pool keeps open. The mariadb driver defaults this
	// to `connectionLimit`, which means idleTimeout NEVER reaps anything -- a
	// pool grows to its limit and holds it for the life of the process. Leave
	// undefined for that (unchanged) driver default; set it lower (e.g. 0) to let
	// a long-lived process or a test suite give idle connections back.
	minimumIdle: configMinimumIdle,
	// How long an acquire waits for a free connection before failing with
	// errno 45028. Undefined = driver default (10s).
	acquireTimeout: configAcquireTimeout,
	// Seconds an idle connection may sit before the pool reaps it. Left
	// undefined here ON PURPOSE: the effective default is 600, and it is owned
	// by the DIALECT (lib/dialects/MySQLDialect.js, lib/dialects/PostgresDialect.js).
	// Do not put a literal in this file -- one default per dialect, one owner.
	// See the note at line 40 above for the interaction that makes this matter:
	// `minimumIdle` defaults to `connectionLimit`, so with `minimumIdle` unset
	// there is nothing above the floor for `idleTimeout` to reap.
	idleTimeout: configIdleTimeout,
...
async function dbh(options = { ignoreCachedConnections: false }) {
	...
	const key = connCacheKey({
		dialectName:
			options.dialect === undefined ? configDialect : options.dialect,
		user: options.user === undefined ? dbUser : options.user,
		host: options.host === undefined ? dbHost : options.host,
		db: options.db === undefined ? dbName : options.db,
		port: options.port === undefined ? dbPort : options.port,
		filename:
			options.filename === undefined ? configFilename : options.filename,
	});
