...
// Per root handle: the lock table exists.
 ...
 * not the transaction).
 *
 * It locks a row of a small table, `yass_locks` (one row per key, created on
 * first use), with `SELECT ... FOR UPDATE`. The key's row is inserted outside
...
const lockKey = async (tx, key) => {
	...
	if (!lockTableReady.has(root)) {
		lockTableReady.set(
			root,
			root.pquery(dialect.createLockTableSql(tableSql)).catch((error) => {
