...
class LoadBalancerManager {
	...
	async _initializeLoadBalancer() {
		let LoadBalancerClass = loadBalancerStrategies[this.strategy];

		if (!LoadBalancerClass) {
			console.warn(
				`Warning: Unknown load balancer strategy: ${
					this.strategy
				...
				{
					strategy: this.strategy,
					availableStrategies: Object.keys(loadBalancerStrategies),
				},
			);
			LoadBalancerClass = loadBalancerStrategies.roundRobin;
		}
	...
	async setStrategy(newStrategy, newOptions = {}) {
		...
		let validatedStrategy = newStrategy;

		if (!newStrategy || typeof newStrategy !== 'string') {
			console.warn(
				'Warning: Strategy must be a non-empty string. Falling back to roundRobin.',
				{
					strategy: newStrategy,
				},
			);
			validatedStrategy = 'roundRobin';
		} else if (!(newStrategy in loadBalancerStrategies)) {
			const availableStrategies = this.getAvailableStrategies();
			console.warn(
				`Warning: Invalid strategy: ${newStrategy}. Available strategies: ${availableStrategies.join(
					', ',
				)}. Falling back to roundRobin.`,
				{
					strategy: newStrategy,
			...
			validatedStrategy = 'roundRobin';
		} else if (loadBalancerStrategies[newStrategy] === undefined) {
			console.warn(
				`Warning: Strategy '${newStrategy}' is not available. Use setCustomLoadBalancer() to set a custom strategy first. Falling back to roundRobin.`,
				{
					strategy: newStrategy,
				},
			);
		...
		try {
			...
			this.strategy = validatedStrategy;
