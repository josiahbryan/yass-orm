...
class LoadBalancer {
	 ...
	 * @param {number} [params.query.opts.loadBalancerOptions.queryTimeoutMs] - Query timeout
	 * @param {number} [params.query.opts.loadBalancerOptions.maxRetries] - Retry attempts
	...
	async executeQuery({ targets, query: { sql, params, opts, args } }) {
		...
		if (!conn) {
			throw new Error(
				`[LoadBalancer] No connection available for query: '${sql}' with params: ${JSON.stringify(
					params,
					0,
				)}`,
			);
