...
function mysqlFieldSpec(fieldData, { ignore: ignoreList = [] } = {}) {
	...
	const fieldSpec = `\`${field}\` ${type}${
		...
		!ignoreKey && key === 'PRI'
			...
			: key === 'UNI'
			? ' UNIQUE'
			: '' // TODO: Support other varients of Key if needed
	}${
...
async function mysqlSchemaUpdate(db, table, fieldList, opts) {
	...
	const explainSql = `explain \`${table}\``;
	let error;
	const existingSchema = await execQuery(explainSql).catch((ex) => {
		error = ex;
	});
	if (error) {
		console.warn(`Error updating tale ${table}:`, error);
		return;
	}
	...
	if (existingSchema && existingSchema.length) {
		const explainMap = {};
	...
	// Assume table DOES NOT exist - create
		...
		buff.push(fields.map(mysqlFieldSpec).join(', '));
