...
function mysqlFieldSpec(fieldData, { ignore: ignoreList = [] } = {}) {
	...
	const {
		...
		extra,
		collation,
		_description, // From chainable type metadata
	} = fieldData;
	const { key: ignoreKey } = ignoreMap;

	// Build the base field specification
	let fieldSpec = `\`${field}\` ${type}${
		...
		// Skip DEFAULT for longtext columns - MySQL/MariaDB doesn't support DEFAULT on TEXT/BLOB types
		// The default value is still stored in schema metadata for runtime use by the ORM
		defaultVal !== undefined && !type.match(/^longtext/i)
	...
	// Add COMMENT clause if _description is provided
	// This adds self-documenting database schemas
	if (_description) {
		// Escape single quotes in the description for SQL safety
		const escapedDescription = _description.replace(/'/g, "''");
		fieldSpec += ` COMMENT '${escapedDescription}'`;
	}
...
async function checkJsonSupport() {
	...
	// Test for support of JSON syntax - if not supported, we will not attempt to create any JSON indexes to avoid errors
	let jsonSupported = false;
	const randColNameForJsonTest = `json_test_${Math.random()
		.toString(36)
		.substring(2, 15)}`;
	await execQuery(
		// This should generate "Unknown column" error if server DOES support JSON.
		// Any other error will flag as JSON not supported
		`select ((CAST(${randColNameForJsonTest}->>"${randColNameForJsonTest}" as CHAR(255)) COLLATE utf8mb4_bin))`,
	).catch((ex) => {
		if (ex.message.includes('Unknown column')) {
			console.log(
				`JSON test passed, appears to be supported in your database, we will sync JSON indexes`,
			);
			jsonSupported = true;
		} else {
			// I don't think we need to inform them unless they actually do use JSON indexes.
			// And we do log that below...
			// console.error(
			// 	`JSON appears to NOT be supported in your database, we will not sync JSON indexes`,
			// );
		}
	});
