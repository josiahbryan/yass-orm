...
import { dbh as factory, parseIdField } from './dbh.js';

import { convertDefinition } from './def-to-schema.js';

const convertFile = async (pathname) =>
	convertDefinition(await import(path.resolve(process.cwd(), pathname)));
...
async function uploadMatchRatioFunctionFactory(host, db, user, pass, port) {
				...
				Object.keys(a).forEach((k) => {
					...
					if (ak !== bk) {
						...
						if (
							...
							(k === 'collation' && ak === 'utf8mb4_0900_ai_ci' && bk === '') ||
...
const uploadMatchRatioFunction = () => {
	uploadMatchRatioFunctionFactory(DB_HOST, DB_NAME, USER, PASSWORD, PORT);
};
...
export {
	...
	convertFile,
	dumpDatabaseSchemas,
	uploadMatchRatioFunctionFactory,
