/* eslint-disable no-console, global-require, no-restricted-syntax, no-continue, import/no-unresolved, no-param-reassign */
/**
...
const { BaseDialect, sqlText } = require('./BaseDialect.js');
const { requireOptional } = require('../optional-dependency.js');

const FEATURE = 'the SQLite dialect';

// better-sqlite3, and node-sql-parser under the SQL transformer, are optional
// peer dependencies: loaded on first use, so other dialects run without them.
const getBetterSqlite3 = () =>
	requireOptional('better-sqlite3', { feature: FEATURE });
// Memoized: transformSql runs on every query.
let transformSqlForSqlite;
const getSqliteTransformer = () => {
	if (!transformSqlForSqlite) {
		// eslint-disable-next-line global-require
		({
			transformSqlForSqlite,
		} = require('../sql-transform/SQLiteSqlTransformer.js'));
	}
	return transformSqlForSqlite;
};
...
function normalizeSqliteRow(row) {
	if (typeof row === 'bigint') {
		return normalizeSqliteInteger(row);
	}
	...
	return Object.fromEntries(
		Object.entries(row).map(([key, value]) => [
			key,
			normalizeSqliteInteger(value),
		]),
	);
}

function normalizeSqlitePreparedStatement(stmt) {
	let proxy;
	const normalizedMethods = {
		all(...args) {
			return stmt.all(...args).map(normalizeSqliteRow);
		},
		get(...args) {
			return normalizeSqliteRow(stmt.get(...args));
		},
		iterate(...args) {
			const iterator = stmt.iterate(...args);
			return (function* normalizedRows() {
	...
	proxy = new Proxy(stmt, {
		get(target, prop) {
			if (Object.prototype.hasOwnProperty.call(normalizedMethods, prop)) {
				return normalizedMethods[prop];
			}
...
class SQLiteDialect extends BaseDialect {
	...
	/** SQLite date modifiers: `datetime(expr, '+15 minutes')`. */
	...
	nullSafeEqualSql(a, b, negate = false) {
		return `(${a} IS ${negate ? 'NOT ' : ''}${b})`;
	}

	/** One writer at a time: there are no row locks to take. */
	...
	transformSql(sql, params) {
		const transformed = getSqliteTransformer()({ sql, params });
		return transformed.sql;
	}

	// ============================================
	// Idempotent / Upsert SQL (SQLite native syntax, requires 3.24+)
	// ============================================
	...
	buildInsertIgnoreSql({
		...
		// SQLite's `ON CONFLICT DO NOTHING` swallows ONLY unique/PK
		// conflicts — CHECK, NOT NULL, FK still throw, which matches the
		// method's contract. firstColumnSql is unused here (it's the MySQL
		// no-op SET workaround). conflictColumns can be omitted because
		// SQLite accepts an unconstrained `DO NOTHING` clause.
	...
	buildUpsertSql({
		...
		if (!conflictColumns || conflictColumns.length === 0) {
			throw new Error(
				'SQLite upsert requires conflictColumns (the UNIQUE/PK columns to match on).',
			);
		}
	...
	async getTableColumns(handle, tableName) {
		...
		return rows.map((row) => ({
			name: row.name,
			type: row.type,
			nullable: normalizeSqliteInteger(row.notnull) === 0,
			defaultValue: row.dflt_value,
			primaryKey: normalizeSqliteInteger(row.pk) > 0,
			// SQLite INTEGER PRIMARY KEY is auto-increment by default (ROWID alias)
			autoIncrement:
				normalizeSqliteInteger(row.pk) > 0 &&
				row.type.toUpperCase() === 'INTEGER',
			_raw: normalizeSqliteRow(row),
		}));
	}

	async getTableIndexes(handle, tableName) {
		const indexList = handle
		...
		for (const idx of indexList) {
			...
			indexes.push({
				...
				// Predicate of a PARTIAL index, so schema-sync can detect a CHANGED
				// one. SQLite stores the original CREATE statement verbatim, so unlike
				// Postgres there is no re-rendering to undo.
				where:
					idx.partial === 1
						? (`${indexDefinitions[idx.name] || ''}`.match(
								/\)\s*WHERE\s+(.+)$/is,
						  ) || [])[1]
						: undefined,
	 ...
	 */
	wrapConnection(db) {
		// createConnection() already returns a wrapper, while dbh() also calls
		// wrapConnection(). Keep wrapping idempotent so `_db` always points at
		// the actual better-sqlite3 Database rather than another wrapper.
		if (db && db._db) return db;

		// better-sqlite3 is synchronous; answers like mariadb: rows for a read,
		// { affectedRows, insertId } for a write. Takes mariadb's { sql }
		// options object too (a transaction's connection calls it directly).
		const executeQuery = (sqlOrOptions, params) => {
			const sql = sqlText(sqlOrOptions);
			...
			if (
				...
				return Promise.resolve(stmt.all(...args).map(normalizeSqliteRow));
			}

			const result = stmt.run(...args);
			return Promise.resolve({
				affectedRows: result.changes,
				insertId: normalizeSqliteInteger(result.lastInsertRowid),
			});
		};

		return this.createConnectionWrapper({
			label: 'SQLite',
			props: { _db: db, _queryDirect: executeQuery },
			run: executeQuery,
			end() {
				db.close();
				return Promise.resolve();
			...
			methods: {
				// Waits for an active transaction (SQLite has one connection).
				query(sqlOrOptions, params) {
					if (this._activeTransaction) {
						return this._activeTransaction.then(() =>
							executeQuery(sqlOrOptions, params),
						);
					}
				...
				prepare(sql) {
					return normalizeSqlitePreparedStatement(db.prepare(sql));
				},
	...
	get supportedIsolationLevels() {
		return ['read uncommitted', 'serializable'];
	}

	get supportedTransactionModes() {
		return ['deferred', 'immediate', 'exclusive'];
	}
	...
	get defaultFindOrCreateTransactionOptions() {
		return { mode: 'immediate', maxRetries: 2 };
	}

	async acquireTransactionConnection(handle) {
		// SQLite has one connection rather than a pool. Queue top-level
		// transactions so concurrent async callbacks cannot issue a second BEGIN
		// while the first transaction is still active.
		const ready = handle._transactionQueue || Promise.resolve();
		let unlock;
		handle._transactionQueue = new Promise((resolve) => {
			unlock = resolve;
		});
		await ready;
		let finishActiveTransaction;
		const activeTransaction = new Promise((resolve) => {
			finishActiveTransaction = resolve;
		});
		handle._activeTransaction = activeTransaction;
		const connection = Object.create(handle);
		connection.query = handle._queryDirect;
		return {
			connection,
			release: async () => {
				if (handle._activeTransaction === activeTransaction) {
					delete handle._activeTransaction;
				}
	...
	async beginTransaction(connection, options) {
		const readUncommittedRows = await connection.query(
			'PRAGMA read_uncommitted',
		);
		const queryOnlyRows = await connection.query('PRAGMA query_only');
		const state = {
			readUncommitted: readUncommittedRows[0].read_uncommitted,
			queryOnly: queryOnlyRows[0].query_only,
		};

		try {
			if (options.isolationLevel) {
				connection._db.pragma(
					`read_uncommitted = ${
						options.isolationLevel === 'read uncommitted' ? 1 : 0
					}`,
				);
			}
			if (options.readOnly !== undefined) {
				connection._db.pragma(`query_only = ${options.readOnly ? 1 : 0}`);
			}
			await connection.query(
				`BEGIN ${(options.mode || 'deferred').toUpperCase()}`,
			);
			return state;
		} catch (err) {
			await this.cleanupTransaction(connection, state);
			throw err;
		}
	}

	async cleanupTransaction(connection, state) {
		if (!state) return;
		connection._db.pragma(`read_uncommitted = ${state.readUncommitted}`);
		connection._db.pragma(`query_only = ${state.queryOnly}`);
	}
	...
	get supportsPartialIndexes() {
		return true; // CREATE INDEX ... WHERE <predicate> (SQLite 3.8.0+)
	}
	...
	get supportsTriggers() {
		// SQLite supports triggers as a database capability.
		return true;
	}

	get supportsUuidIdTrigger() {
		// No `uuid()` function in SQLite. UUIDs are generated in JS instead.
		return false;
	}

	get supportsDeclaredTriggers() {
		// Deferred until a schema needs it. sqlite_master.sql stores the
		// verbatim CREATE TRIGGER, so drift comparison is straightforward
		// when the time comes -- just no live consumer today.
		return false;
	}
