...
// Case- and accent-exact comparison, for `exact` string fields.
const EXACT_COLLATION = 'utf8mb4_bin';

const UTC_SESSION_SQL = "SET time_zone = '+00:00'";

/** Longest table, index, trigger or function name MySQL accepts (characters). */
const MYSQL_MAX_IDENTIFIER_LENGTH = 64;

/**
 * The mariadb driver's time zone options.
 *
 * yass writes datetimes as UTC wall clocks (lib/dbh.js deflateValue), so by
 * default the driver converts them as UTC ('Etc/GMT+0') without touching the
 * session (skipSetTimezone). `disableTimezone` sends neither (PlanetScale).
 *
 * `timezone: 'utc'` (opt-in) also runs `SET time_zone = '+00:00'` on every new
 * connection, so the server's NOW(), CURRENT_TIMESTAMP and friends agree with
 * what yass writes. Without it they use the server's zone.
 *
 * On a pool, `timezone: 'utc'` also turns off `resetAfterUse`: on MariaDB
 * 10.2.22+/10.3.13+ the driver sends COM_RESET_CONNECTION on every release
 * (pool.query included), which puts the session time zone back to the
 * server's, so only each connection's first query would have run in UTC. With
 * it off, release rolls back instead, which is what MySQL servers always get.
 *
 * @param {{ timezone?: string, disableTimezone?: boolean }} config
 * @param {{ pool?: boolean }} [target]
 * @returns {object} options to spread into the driver config
 */
function driverTimezoneOptions({ timezone, disableTimezone }, { pool } = {}) {
	// dbh() rejects any other value from config. Anything else passed straight
	// to the dialect is ignored, as it always was.
	const utcSession = `${timezone}`.toLowerCase() === 'utc';
	if (disableTimezone) {
		if (utcSession) {
			throw new Error(
				"[yass-orm] `timezone: 'utc'` and `disableTimezone` contradict each other: set one.",
			);
		}
		return {};
	}
	return {
		...
		...(utcSession ? { initSql: UTC_SESSION_SQL } : {}),
		...(utcSession && pool ? { resetAfterUse: false } : {}),
	};
}
...
// A regular dependency (MySQL is the default dialect), loaded on first use all
// the same, so a SQLite- or Postgres-only process never opens it.
const getMariaDb = () =>
	requireOptional('mariadb', { feature: 'the MySQL dialect' });
...
class MySQLDialect extends BaseDialect {
	...
	/** MySQL's own spelling, `LIMIT offset, count` (what finder.js always sent). */
	limitSql(limit, offset = 0) {
		return `LIMIT ${parseInt(offset || 0, 10)}, ${parseInt(limit, 10)}`;
	}
	...
	/**
	 * UTC_TIMESTAMP(), not NOW(): yass writes UTC, and NOW() is in the
	 * session's time zone (the server's, unless `timezone: 'utc'`).
	 */
	nowSql() {
	...
	intervalSql(expr, amount, unit, subtract = false) {
		return `${
			subtract ? 'DATE_SUB' : 'DATE_ADD'
		}(${expr}, INTERVAL ${amount} ${unit.toUpperCase()})`;
	}

	nullSafeEqualSql(a, b, negate = false) {
		return `${negate ? 'NOT ' : ''}(${a} <=> ${b})`;
	}

	/** MySQL sorts NULLs first ascending and has no NULLS LAST. */
	nullsLastSql(expr, direction) {
		return `${expr} IS NULL, ${expr} ${direction}`;
	}

	/** Case- and accent-exact names (utf8mb4_bin), like other identity columns. */
	createLockTableSql(tableSql) {
		return `CREATE TABLE IF NOT EXISTS ${tableSql} (name VARCHAR(191) NOT NULL PRIMARY KEY) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin`;
	}

	tableExistsHereSql() {
		return 'SELECT 1 AS present FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = :name';
	}
	...
	/**
	 * `precision: n` -> DATETIME(n); `exact` -> COLLATE utf8mb4_bin (the
	 * server default, utf8mb4_0900_ai_ci, ignores case AND accents). Any other
	 * explicit `collation` is kept as it is. See BaseDialect.physicalField.
	...
	physicalField(fieldData) {
		const out = { ...fieldData };
		if (out.precision && `${out.type}`.toLowerCase() === 'datetime') {
			out.type = `datetime(${out.precision})`;
		}
		if (out.exact) {
			if (out.collation && out.collation !== EXACT_COLLATION) {
				throw new Error(
					`Field '${out.field}' is exact, which needs collation ${EXACT_COLLATION}, ` +
						`but also sets collation '${out.collation}'. Use one or the other.`,
				);
			}
			out.collation = EXACT_COLLATION;
		}
		return out;
	}
	...
	async createPool(config) {
		...
		const pool = await db.createPool({
			...
			bigNumberStrings: true,
			...(config.ssl ? { ssl: config.ssl } : {}),
			...driverTimezoneOptions(config, { pool: true }),
		});
