...
class PostgresDialect extends BaseDialect {
	...
	generateCreateIndex(tableName, indexName, columns, options = {}) {
		...
		if (fulltext) {
			const tsvectorColumns = columns
				...
				.join(" || ' ' || ");
			return `CREATE INDEX ${quotedIndex} ON ${quotedTable} USING GIN (to_tsvector('english', ${tsvectorColumns}))`;
		}
	...
	generateAlterModifyColumn(tableName, fieldData) {
		...
		const type = this.mapType(fieldData.type);
		// PostgreSQL uses ALTER COLUMN ... TYPE (not CHANGE like MySQL)
		return `ALTER TABLE ${quotedTable} ALTER COLUMN ${quotedField} TYPE ${type}`;
	}
	...
	async createConnection(config) {
		...
		return this.wrapConnection(client);
	}
	...
	async createPool(config) {
		...
		return this.wrapConnection(pool);
	}
