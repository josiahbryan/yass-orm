...
function normalizePostgresJsonAccessor(col) {
	// Both quote styles appear in real schemas; Postgres needs a single-quoted
	// string literal (double quotes would be an identifier).
	return `${col}`.replace(
		/(->>?)\s*['"](?:\$\.)?([^'"]+)['"]/g,
		(match, operator, key) => `${operator}'${key}'`,
	);
}
		...
		return rows
			...
			.map((row) => {
				...
				if (isFullText) {
					...
					columns = [
						normalizePostgresIndexExpression(defMatch ? defMatch[1] : ''),
					];
				} else {
