...
describe('PostgresDialect', () => {
	...
	describe('SQL Syntax & Formatting', () => {
		...
		describe('prepareParams()', () => {
			...
			it('should deflate Date objects to ISO strings', () => {
				...
				const result = dialect.prepareParams(params, paramOrder);
				// The instant, not a naive UTC wall clock that a TIMESTAMPTZ
				// column would read in the session's time zone.
				expect(result[0]).to.equal('2024-01-15T10:30:00.000Z');
			});
	...
	describe('Type Mapping', () => {
		describe('mapType()', () => {
			...
			it('should map date/time types', () => {
				...
				// t.datetime is an absolute instant on Postgres
				expect(dialect.mapType('datetime')).to.equal('TIMESTAMPTZ');
			...
			// `t.string` converts to the BARE type `varchar` with no length. MySQL
			// normalizes that to varchar(255) in generateFieldSpec; without the same
			// normalization here it hit the `|| 'TEXT'` fallback, so every t.string
			// column silently became TEXT instead of VARCHAR(255).
			it('should map a bare varchar (what t.string converts to) to VARCHAR(255)', () => {
				expect(dialect.mapType('varchar')).to.equal('VARCHAR(255)');
				expect(dialect.mapType('varchar(-1)')).to.equal('VARCHAR(255)');
				expect(dialect.mapType('nvarchar(-1)')).to.equal('VARCHAR(255)');
			});
	...
	describe('DDL Generation', () => {
		describe('generateCreateTable()', () => {
			...
			// `t.object` (and its `t.array` alias) convert to `{ type: 'longtext',
			// isObject: true }` -- the MySQL storage type. On Postgres that landed in
			// TEXT, throwing away JSONB's native operators and indexing for the one
			// field type that most wants them. `t.json` already reached JSONB, so the
			// two spellings disagreed about where the same data belongs.
			describe('uuid link columns (uuidLinkedIds)', () => {
				it('creates a CHAR(36) link column as native UUID so it joins to its uuidKey', () => {
					expect(
						dialect.generateFieldSpec({
							field: 'parent',
							type: 'char(36)',
					...
					).to.equal('"parent" UUID');
				});

				it('leaves a plain t.uuid (no linkedModel) field as CHAR(36)', () => {
					expect(
						dialect.generateFieldSpec({ field: 'token', type: 'char(36)' }),
					).to.equal('"token" CHAR(36)');
				});

				it('leaves an int link column (uuidLinkedIds off) as INTEGER', () => {
					expect(
						dialect.generateFieldSpec({
							field: 'parent',
							type: 'int',
					...
					).to.equal('"parent" INTEGER');
				});

				it('casts explicitly when ALTERing a link column to UUID (blank -> NULL)', () => {
					const sql = dialect.generateAlterModifyColumn('child', {
						field: 'parent',
						type: 'char(36)',
						linkedModel: 'parent',
					});
					expect(sql).to.contain(
						`ALTER TABLE "child" ALTER COLUMN "parent" TYPE UUID USING NULLIF(TRIM("parent"::text), '')::uuid`,
					);
				});
			});

			describe('t.object / t.array -> JSONB', () => {
				it('maps an isObject field to JSONB, not TEXT', () => {
					const spec = dialect.generateFieldSpec({
						field: 'data',
						type: 'longtext',
						isObject: true,
					});
					expect(spec).to.equal('"data" JSONB');
				});

				it('maps an isObject array field to JSONB too', () => {
					const spec = dialect.generateFieldSpec({
						field: 'tags',
						type: 'longtext',
						isObject: true,
					...
					expect(spec).to.equal('"tags" JSONB');
				});

				it('leaves a plain t.text field as TEXT', () => {
					const spec = dialect.generateFieldSpec({
						field: 'body',
						type: 'longtext',
					});
					expect(spec).to.equal('"body" TEXT');
				});

				// Postgres will not cast text to jsonb implicitly, so an ALTER on a
				// column created under the old TEXT mapping needs an explicit USING.
				it('emits an explicit USING cast when altering to JSONB', () => {
					const sql = dialect.generateAlterModifyColumn('items', {
						field: 'data',
						type: 'longtext',
						isObject: true,
					});
					expect(sql).to.include('TYPE JSONB USING "data"::jsonb');
				});
			});
		...
		describe('generateCreateIndex()', () => {
			...
			// Schemas spell JSON paths MySQL-style ($.key). Postgres' ->> takes a KEY
			// NAME, so emitting that verbatim built an index on a key literally called
			// '$.email' -- valid DDL, so it succeeded silently and indexed NULL
			// forever. And since PostgresSqlTransformer strips `$.` on the query side,
			// the planner could never match the index to a query.
			describe('JSON path normalization', () => {
				it("strips the MySQL '$.' prefix so the key is real", () => {
					const sql = dialect.generateCreateIndex('users', 'idx_json', [
						"data->>'$.email'",
					]);
					expect(sql).to.include(`(data->>'email')`);
					expect(sql).to.not.include('$.');
				});

				it('converts double-quoted paths to single-quoted PG literals', () => {
					// In Postgres a double-quoted token is an IDENTIFIER, not a string.
					const sql = dialect.generateCreateIndex('users', 'idx_json', [
						'data->>"$.email"',
					]);
					expect(sql).to.include(`(data->>'email')`);
					expect(sql).to.not.include('"$.email"');
				});

				// If these two ever disagree the index is dead weight the planner can
				// never match. Compared with whitespace removed, since spacing is
				// semantically irrelevant and only the accessor itself must agree.
				// Identifier quoting also differs (the query path quotes `"meta"`, the
				// index path does not) and is likewise irrelevant -- only the operator
				// and the path literal have to agree.
				const accessorOf = (sql) => {
					const stripped = `${sql}`.replace(/\s+/g, '').replace(/"/g, '');
					const match = stripped.match(/(meta(?:->>|#>>)'[^']+')/);
					return match ? match[1] : null;
				};

				it('emits the same accessor the query transformer produces', () => {
					const { sql: querySql } = dialect.transformSql(
						"SELECT * FROM t WHERE meta->>'$.valence' = :v",
						{ v: 'x' },
					);
					const indexSql = dialect.generateCreateIndex('t', 'idx', [
						"meta->>'$.valence'",
					]);
					expect(accessorOf(querySql)).to.equal(`meta->>'valence'`);
					expect(accessorOf(indexSql)).to.equal(accessorOf(querySql));
				});

				it('agrees on the NESTED accessor too (#>> with a path literal)', () => {
					const { sql: querySql } = dialect.transformSql(
						"SELECT * FROM t WHERE meta->>'$.a.b' = :v",
						{ v: 'x' },
					);
					const indexSql = dialect.generateCreateIndex('t', 'idx', [
						"meta->>'$.a.b'",
					]);
					expect(accessorOf(querySql)).to.equal(`meta#>>'{a,b}'`);
					expect(accessorOf(indexSql)).to.equal(accessorOf(querySql));
				});

				it('normalizes an introspected functional index back to schema shorthand', async () => {
					// pg_get_indexdef renders it as ((meta ->> 'valence'::text)); without
					// normalizing, the signature never matches and the index is dropped
					// and recreated on every sync.
					const handle = {
						async query() {
							return {
								rows: [
									{
										...
										is_unique: false,
										is_primary: false,
										index_def: `CREATE INDEX idx_json ON public.t USING btree (((meta ->> 'valence'::text)))`,
									},
								],
							};
						},
					};
					const [idx] = await dialect.getTableIndexes(handle, 't');
	...
	describe('Feature Flags', () => {
		...
		it('supports triggers as a database capability', () => {
			// The database itself supports triggers; specific yass-orm features
			// (UUID id trigger, declared-trigger reconciler) may not be
			// implemented for Postgres yet -- see below.
			expect(dialect.supportsTriggers).to.be.true;
		});

		it('does NOT support the UUID id trigger (MySQL-specific uuid() syntax)', () => {
			expect(dialect.supportsUuidIdTrigger).to.be.false;
		});

		it('does NOT support declared-trigger reconciliation yet (needs CREATE FUNCTION + pg_get_triggerdef normalizer)', () => {
			expect(dialect.supportsDeclaredTriggers).to.be.false;
		});
	...
	describe('generateCreateIndex() FULLTEXT expression parenthesization', () => {
		...
		// The text-search config selects stemming/stopword rules, so it is a schema
		// decision. It was hardcoded to 'english', which no deployment could change.
		it('honors a per-index textSearchConfig', () => {
			const sql = dialect.generateCreateIndex(
				'articles',
				'idx_ft_body',
				['body'],
				{ fulltext: true, textSearchConfig: 'spanish' },
			);
			expect(sql).to.include(`to_tsvector('spanish', "body")`);
			expect(sql).to.not.include('english');
		});

		it("defaults to 'english' when none is given", () => {
			expect(dialect.defaultTextSearchConfig).to.equal('english');
			const sql = dialect.generateCreateIndex(
				'articles',
				'idx_ft_body',
				['body'],
		...
		// The value lands in DDL as a literal, so a schema file must not be able to
		// smuggle SQL through it.
		it('rejects a textSearchConfig that is not a plain identifier', () => {
			expect(() =>
				dialect.resolveTextSearchConfig("english'); DROP TABLE x --"),
			).to.throw(/Invalid Postgres textSearchConfig/);
			expect(() => dialect.resolveTextSearchConfig('simple')).to.not.throw();
		});

		it('reads the config back off an existing index definition', async () => {
			const handle = {
				async query() {
					return {
						rows: [
							{
								index_name: 'idx_ft_body',
								is_unique: false,
								is_primary: false,
								index_def: `CREATE INDEX idx_ft_body ON public.articles USING gin (to_tsvector('spanish'::regconfig, body))`,
							},
						],
					};
				},
			...
			const [idx] = await dialect.getTableIndexes(handle, 'articles');
			// Without this, switching languages would leave the old index in place:
			// same columns, same flags, so the signature would not change.
			expect(idx.textSearchConfig).to.equal('spanish');
		});
