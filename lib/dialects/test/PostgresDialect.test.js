	...
	describe('DDL Generation', () => {
		...
		describe('generateCreateIndex()', () => {
			...
			describe('JSON path normalization', () => {
				...
				it('emits the same expression the query transformer produces', () => {
					// If these two ever disagree the index is unusable dead weight.
					const { sql: querySql } = dialect.transformSql(
						"SELECT * FROM t WHERE meta->>'$.valence' = :v",
						{ v: 'x' },
					);
					expect(querySql).to.include(`->> 'valence'`);
					const indexSql = dialect.generateCreateIndex('t', 'idx', [
						"meta->>'$.valence'",
					]);
					expect(indexSql).to.include(`meta->>'valence'`);
				});
