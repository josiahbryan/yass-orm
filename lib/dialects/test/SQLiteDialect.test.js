...
describe('SQLiteDialect', () => {
	...
	describe('Type Mapping', () => {
		describe('mapType()', () => {
			...
			it('should map bigint to INTEGER', () => {
				expect(dialect.mapType('bigint')).to.equal('INTEGER');
			});
	...
	describe('Feature Flags', () => {
		...
		it('supports triggers as a database capability', () => {
			// SQLite does support CREATE TRIGGER; individual yass-orm features
			// on top of that may or may not be implemented -- see below.
			expect(dialect.supportsTriggers).to.be.true;
		});

		it('does NOT support the UUID id trigger (no uuid() function; UUIDs generated in JS)', () => {
			expect(dialect.supportsUuidIdTrigger).to.be.false;
		});

		it('does NOT support declared-trigger reconciliation yet (deferred until a schema needs it)', () => {
...
describe('SQLiteDialect Integration', function () {
	...
	describe('Connection Management', () => {
		...
		it('should request safe integer results so bigint fields can be stringified without precision loss', async function () {
			if (!BetterSqlite3Available) {
				this.skip();
				return;
			}
			...
			db = await dialect.createConnection({ filename: ':memory:' });
			await db.query('CREATE TABLE big_numbers (value INTEGER)');
			await db.pquery('INSERT INTO big_numbers (value) VALUES (:value)', {
				value: BigInt('9007199254740993'),
			});

			const [row] = await db.pquery('SELECT value FROM big_numbers');
			expect(typeof row.value).to.equal('bigint');
			expect(row.value.toString()).to.equal('9007199254740993');
		});

		it('should normalize safe integers returned from prepared statements', async function () {
			if (!BetterSqlite3Available) {
				this.skip();
				return;
			}
			...
			db = await dialect.createConnection({ filename: ':memory:' });
			await db.query(
				'CREATE TABLE prepared_numbers (safe INTEGER, unsafe INTEGER)',
			);
			await db.pquery(
				'INSERT INTO prepared_numbers (safe, unsafe) VALUES (:safe, :unsafe)',
				{
					safe: 25,
					unsafe: BigInt('9007199254740993'),
				},
			);

			const stmt = db.prepare('SELECT safe, unsafe FROM prepared_numbers');
			const rowFromGet = stmt.get();
			const [rowFromAll] = stmt.all();

			expect(rowFromGet.safe).to.equal(25);
			expect(typeof rowFromGet.unsafe).to.equal('bigint');
			expect(rowFromGet.unsafe.toString()).to.equal('9007199254740993');
			expect(rowFromAll.safe).to.equal(25);
			expect(typeof rowFromAll.unsafe).to.equal('bigint');
			expect(rowFromAll.unsafe.toString()).to.equal('9007199254740993');
		});

		it('should preserve prepared statement helper methods while normalizing results', async function () {
			if (!BetterSqlite3Available) {
				this.skip();
				return;
			}
			...
			db = await dialect.createConnection({ filename: ':memory:' });
			await db.query('CREATE TABLE prepared_helpers (safe INTEGER)');
			await db.pquery('INSERT INTO prepared_helpers (safe) VALUES (:safe)', {
				safe: 25,
			});

			const stmt = db.prepare('SELECT safe FROM prepared_helpers');
			expect(stmt.columns()[0].name).to.equal('safe');
			expect(stmt.pluck().get()).to.equal(25);
		});
	...
	describe('Schema Introspection', () => {
		...
		it('reports a partial index and its predicate', async function () {
			if (!BetterSqlite3Available) {
				this.skip();
				return;
			}
			// Created here rather than in the shared fixture, so the index COUNT other
			// introspection tests assert stays as it was.
			// SQLite supports PARTIAL indexes (3.8.0+); schema-sync compares the
			// predicate to detect a CHANGED one, so introspection has to report it.
			await db.query(
				'CREATE INDEX idx_schema_partial ON schema_test (name) WHERE age > 18',
			);
			const indexes = await dialect.getTableIndexes(db, 'schema_test');
			const byName = {};
			indexes.forEach((i) => {
				byName[i.name] = i;
			});

			const partial = byName.idx_schema_partial;
			expect(partial).to.not.equal(undefined);
			expect(partial.partial).to.equal(true);
			// Unlike Postgres, SQLite stores the statement verbatim -- no re-rendering
			// to undo -- so the predicate comes back as written.
			expect(`${partial.where}`.trim()).to.equal('age > 18');
			// The WHERE clause must not leak into the column list.
			expect(partial.columns).to.deep.equal(['name']);

			// An ordinary index carries no predicate.
			expect(byName.idx_schema_name.partial).to.equal(false);
			expect(byName.idx_schema_name.where).to.equal(undefined);
		});
