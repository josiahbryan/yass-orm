...
describe('MySQLDialect', () => {
	...
	describe('Connection Management', () => {
		it('should request BIGINT-safe string results from the MariaDB pool', async () => {
			const mariadb = require('mariadb');
			const originalCreatePool = mariadb.createPool;
			let capturedConfig;

			mariadb.createPool = async (poolConfig) => {
				capturedConfig = poolConfig;
				return { query: async () => [] };
			};
			...
			expect(capturedConfig.supportBigNumbers).to.equal(true);
			expect(capturedConfig.bigNumberStrings).to.equal(true);
		});
	});
	...
	describe('Feature Flags', () => {
		...
		it('should support the UUID id trigger (yass-orm auto-generates it)', () => {
			expect(dialect.supportsUuidIdTrigger).to.be.true;
		});

		it('should support DECLARED trigger reconciliation (author-facing `triggers` block)', () => {
			expect(dialect.supportsDeclaredTriggers).to.be.true;
		});
	...
	describe('Transactions', () => {
		it('leases one pool connection and applies isolation/read-only options', async () => {
			const calls = [];
			const leased = {
				query: async (sql) => calls.push(sql.sql || sql),
				commit: async () => calls.push('driver commit'),
				rollback: async () => calls.push('driver rollback'),
				release: async () => calls.push('driver release'),
			};
			const pool = { getConnection: async () => leased };

			const lease = await dialect.acquireTransactionConnection(pool);
	...
	// ============================================
	// Trigger DDL and introspection
	// ============================================
	//
	// The DDL generators are pure string builders (no DB required) so they can
	// be pinned exactly here. The introspection method is exercised with a
	// mock `handle` that simulates the exact rowset shape MySQL returns from
	// information_schema.TRIGGERS -- the LIVE round-trip is the acceptance
	// test in test/schemaSync.triggers.test.js.

	describe('generateDropTrigger()', () => {
		it('emits an IF EXISTS drop (idempotent by construction)', () => {
			expect(dialect.generateDropTrigger({ name: 'set_nonce_upper' })).to.equal(
				'DROP TRIGGER IF EXISTS `set_nonce_upper`',
			);
		});

		it('qualifies with database when provided (cross-db syncs)', () => {
			expect(
				dialect.generateDropTrigger({
					name: 'set_nonce_upper',
					database: 'testdb',
				}),
			).to.equal('DROP TRIGGER IF EXISTS `testdb`.`set_nonce_upper`');
		});
	});

	describe('generateCreateTrigger()', () => {
		it('emits `CREATE TRIGGER <name> <TIMING> <EVENT> ON <table> FOR EACH ROW <body>`', () => {
			// The engine owns the ON clause + table quoting so the author only
			// writes the body. UP-CASING TIMING/EVENT keeps the emitted DDL and
			// the information_schema round-trip byte-for-byte comparable.
			const sql = dialect.generateCreateTrigger({
				name: 'set_nonce_upper',
				timing: 'before',
				event: 'insert',
				tableName: 'yass_test2',
				body: `BEGIN
					IF new.nonce IS NULL THEN SET new.nonce = UPPER(new.name); END IF;
				END`,
			});
			expect(sql).to.match(
				/^CREATE TRIGGER `set_nonce_upper` BEFORE INSERT ON `yass_test2` FOR EACH ROW\s+BEGIN/,
			);
			expect(sql).to.contain('SET new.nonce = UPPER(new.name)');
			expect(sql).to.contain('END');
		});

		it('emits FOLLOWS <name> when passed a `follows` argument (ordering inside a group)', () => {
			// MySQL fires same-timing+event triggers in creation order. A DROP+
			// CREATE otherwise moves a trigger to the END of the group; passing
			// `follows` pins it behind a specific sibling so the reconciler can
			// rebuild a group in a chosen order.
			const sql = dialect.generateCreateTrigger({
				name: 'hash_row',
				timing: 'before',
				event: 'insert',
				tableName: 'yass_test2',
				body: 'BEGIN END',
				follows: 'before_insert_yass_test2_set_id',
			});
			expect(sql).to.match(
				/BEFORE INSERT ON `yass_test2` FOR EACH ROW FOLLOWS `before_insert_yass_test2_set_id`/,
			);
		});

		it('qualifies BOTH the trigger name and the table with the database when provided', () => {
			// MySQL error 1435 ("Trigger in wrong schema") fires when the
			// trigger name and the table are in different schemas. An
			// unqualified name resolves to the connection's current schema,
			// so a cross-schema DDL that qualifies only the table would
			// error unless the session already `USE`d the right DB -- a
			// hidden dependency this reconciler must not carry. The name
			// gets the same `db.` prefix.
			const sql = dialect.generateCreateTrigger({
				name: 't',
				timing: 'after',
				event: 'update',
			...
			expect(sql).to.contain('CREATE TRIGGER `testdb`.`t`');
			expect(sql).to.contain('ON `testdb`.`yass_test2`');
		});

		it('normalizes lowercase timing/event to the canonical uppercase spelling', () => {
			// MySQL stores ACTION_TIMING/EVENT_MANIPULATION uppercase either
			// way, so writing them lowercase would be correct but sloppy; the
			// reconciler compares case-insensitively but the DDL should read the
			// way a DBA would `SHOW CREATE TRIGGER` see it.
			const sql = dialect.generateCreateTrigger({
				name: 't',
				timing: 'BeFoRe',
				event: 'iNsErT',
			...
			expect(sql).to.contain('BEFORE INSERT');
		});
	});

	describe('getTableTriggers()', () => {
		it('reads information_schema.TRIGGERS and returns { name, timing, event, body, order }', async () => {
			const seenQueries = [];
			const handle = {
				query: async (sql, params) => {
					...
					return [
						{
							TRIGGER_NAME: 'before_insert_t_set_id',
						...
						{
							TRIGGER_NAME: 'hash_row',
							ACTION_TIMING: 'BEFORE',
							EVENT_MANIPULATION: 'INSERT',
							ACTION_STATEMENT: 'BEGIN SET new.hash = SHA1(new.name); END',
							ACTION_ORDER: 2,
						},
					];
				},
			...
			const rows = await dialect.getTableTriggers(handle, 'testdb', 't');

			// Query must use information_schema (SHOW TRIGGERS is current-DB only
			// and cannot serve the `db.table` form), and pass params -- never
			// interpolate.
			expect(seenQueries[0].sql).to.match(/information_schema\.TRIGGERS/i);
			expect(seenQueries[0].params).to.deep.equal(['testdb', 't']);
			...
			expect(rows).to.deep.equal([
				{
					...
					timing: 'BEFORE',
					event: 'INSERT',
					body: 'BEGIN IF new.id IS NULL THEN SET new.id = uuid(); END IF; END',
					order: 1,
				},
				{
					name: 'hash_row',
					timing: 'BEFORE',
					event: 'INSERT',
					body: 'BEGIN SET new.hash = SHA1(new.name); END',
					order: 2,
				},
			]);
		});

		it('returns an empty array when no triggers exist on the table', async () => {
			const handle = { query: async () => [] };
			const rows = await dialect.getTableTriggers(handle, 'testdb', 't');
			expect(rows).to.deep.equal([]);
		});
	});
