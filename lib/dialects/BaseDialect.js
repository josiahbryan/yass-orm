...
class BaseDialect {
	...
	/**
	 * A query for whether the table `:name` exists where an unqualified
	 * CREATE TABLE puts it (a row when it does). A catalog read: no DDL, so
	 * no wait behind transactions that use the table.
	 * @returns {string}
	 */
	tableExistsHereSql() {
		return 'SELECT 1 AS present FROM information_schema.tables WHERE table_schema = current_schema() AND table_name = :name';
	}
