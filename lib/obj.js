...
const { assertKnownFields } = require('./reject-unknown-fields');
...
// Use globalThis so the hook list survives ESM module duplication (same as
// the other ORM caches above).
if (!globalThis.__YASS_ORM_GLOBAL_CHANGE_HOOKS__) {
	globalThis.__YASS_ORM_GLOBAL_CHANGE_HOOKS__ = [];
}
const GLOBAL_CHANGE_HOOKS = globalThis.__YASS_ORM_GLOBAL_CHANGE_HOOKS__;
...
class DatabaseObject {
	...
	/**
	 * @static rejectUnknownFields - When `true`, the CALLER-SUPPLIED objects
	 * passed to `search`/`searchOne`/`findOrCreate`/`create`/`patch`/
	 * `patchIf` must contain only real schema columns — an unrecognized key
	 * throws, naming the key, instead of being silently dropped by
	 * `deflateValues()`. Defaults to `false` so no existing model's behavior
	 * changes; opt in per subclass with `static rejectUnknownFields = true;`.
	 * See BDL-2697.
	 * @type {boolean}
	 */
	static rejectUnknownFields = false;
	 ...
	 */
	static async inflateValues(data, span, promisePoolMapConfig, { tx } = {}) {
	...
	static async _resolvedLinkedModel(
		modelName,
		modelId,
		span = undefined,
		{ tx } = {},
	...
	async _updateProperties(data, span, { tx } = {}) {
		...
		const inflatedData = await this.constructor.inflateValues(
			data,
			span,
			undefined,
			{ tx },
	...
	static async search(
		...
		if (this.rejectUnknownFields) {
			assertKnownFields(fields, this.schema().fieldMap, {
				className: this.name,
				methodName: 'search',
				argName: 'fields',
			});
	...
	static async searchOne(
		...
		return this.search(fields, true, promisePoolMapConfig, { tx });
	}
	...
	static async findOrCreate(
		...
		if (this.rejectUnknownFields) {
			const { fieldMap } = this.schema();
			assertKnownFields(fields, fieldMap, {
				className: this.name,
				methodName: 'findOrCreate',
				argName: 'fields',
			});
			assertKnownFields(patchIf, fieldMap, {
				className: this.name,
				methodName: 'findOrCreate',
				argName: 'patchIf',
			});
			assertKnownFields(patchIfFalsey, fieldMap, {
				className: this.name,
				methodName: 'findOrCreate',
				argName: 'patchIfFalsey',
			});
		}
	...
	static async create(data, { tx } = {}) {
		if (this.rejectUnknownFields) {
			assertKnownFields(data, this.schema().fieldMap, {
				className: this.name,
				methodName: 'create',
				argName: 'data',
			});
	...
	async patchIf(values = {}, ifFalsey = {}) {
		if (this.constructor.rejectUnknownFields) {
			const { fieldMap } = this.constructor.schema();
			assertKnownFields(values, fieldMap, {
				className: this.constructor.name,
				methodName: 'patchIf',
				argName: 'values',
			});
			assertKnownFields(ifFalsey, fieldMap, {
				className: this.constructor.name,
				methodName: 'patchIf',
				argName: 'ifFalsey',
			});
	...
	async patch(data, { tx } = {}) {
		if (this.constructor.rejectUnknownFields) {
			assertKnownFields(data, this.constructor.schema().fieldMap, {
				className: this.constructor.name,
				methodName: 'patch',
				argName: 'data',
			});
