...
class DatabaseObject {
	...
	static async _resolvedLinkedModel(modelName, modelId, span = undefined) {
		const resolvedPath = `${path.resolve(this.basePath(), modelName)}.js`;
		...
		if (!pathExists) {
			...
			throw new Error(
				`Cannot resolve linked model '${modelName}' on table '${this.table()}' (trying to look up ID '${modelId}')${
					errorSpan || ''
				}`,
			);
		}

		let ModelClass = require(resolvedPath);
