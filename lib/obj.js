...
class DatabaseObject {
			...
			this.fields().map(async (row) => {
				...
				if (value === null) {
					...
					try {
						...
						if (row.template) {
							value = Object.assign({}, row.template, value || {});

							// Allow templates to have values like t.linked('user') and we will inflate them here
							await promiseMap(
								Object.entries(row.template),
								async ([key, templateValue]) => {
									const dataValue = value[key];
									if (dataValue) {
										if (templateValue && templateValue.linkedModel) {
											value[key] = await this._resolvedLinkedModel(
												templateValue.linkedModel,
												dataValue,
											);
										}
									}
	...
	static _deflateObjectSchema(deflatedData = {}, row = {}, value = null) {
		if (!value) {
			return value;
		}
		...
		if (row.objectSchema) {
			if (row.template) {
				...
				// Allow templates to have values like t.linked('user') and we deflate it to user.id here
				Object.entries(row.template).forEach(([key, templateValue]) => {
					const dataValue = value[key];
					if (dataValue) {
						const idField =
							templateValue &&
							templateValue.linkedModel &&
							templateValue.linkedModel.idField();

						if (idField && dataValue[idField]) {
							value[key] = dataValue[idField];
						}
					}
				});
			}
			...
			Object.values(row.objectSchema).forEach((subrow) => {
				...
				if (subrow.isObject) {
					deflatedData[subrow.field] = this._deflateObjectSchema(
						deflatedData,
						subrow,
	...
	static deflateValues(object = {}, noUndefined) {
		...
		this.fields().forEach((row) => {
			...
			if (row.linkedModel && value && value[row.linkedModel.idField()]) {
				value = value[idField];
			} else if (row.isObject) {
				// console.log("[base-model.deflate] isObject:", value, row);
				const tmp = this._deflateObjectSchema(deflatedData, row, value);
	...
	remove(/* , params */) {
		...
		if (this.constructor.schema().fieldMap.isDeleted) {
			// TODO: Check linked fields for 'cascadeRemove' and remove ...
