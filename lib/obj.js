...
class DatabaseObject {
	...
	static async _resolveModelClass(modelName, errorHint = '', span = undefined) {
		...
		if (externalPathIndex instanceof Map) {
			// Normalize path by stripping extension for lookup
			const normalizedPath = resolvedPath.replace(/\.(js|ts|cjs|mjs)$/, '');
...
const getCachedDefinition = (basePath, definitionFile) => {
	...
	const { pathToFileURL } = require('url');
	const esmRequire = createRequire(pathToFileURL(resolvedPath).href);
	const definition = esmRequire(resolvedPath);
