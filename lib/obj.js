...
class DatabaseObject {
	...
	static fields() {
		...
		if (!this._cachedFields) {
			...
			this._cachedFields = Object.values(this.schema().fieldMap);
		}
	 ...
	 */
	static idField() {
		if (!this._cachedIdField) {
			const { idField } = parseIdField(this.table()) || { id: 'id' };
			this._cachedIdField = idField;
		}
		return this._cachedIdField;
	}

	idField() {
		if (this.constructor._cachedIdField) {
			return this.constructor._cachedIdField;
		}
	...
	static async _resolveModelClass(modelName, errorHint = '', span = undefined) {
		...
		if (!pathExists) {
			...
			throw new Error(
				`Cannot resolve linked model '${modelName}' (resolved to file path: '${resolvedPath}') on table '${this.table()}' ${errorHint} ${
					errorSpan || ''
				}`,
			);
		}

		const resolvedModel = resolvedPath.endsWith('.js')
	...
	constructor(data, constructorAllowed) {
		...
		this._cachedFields = this.constructor.fields();

		const fields = this._cachedFields;
		for (let i = 0; i < fields.length; i++) {
			const row = fields[i];
...
const getCachedDefinition = (basePath, definitionFile) => {
	...
	MODEL_DEFINITION_CACHE[resolvedPath] = definition;
