...
const { dbh: getDbh, autoFixTable, deflateValue, debugSql, QueryTiming } = require('./dbh');
const {
	finder,
	// filterData,
	// promiseFilter
...
class DatabaseObject {
		...
		await Promise.all(this.fields().map(async row => {
			...
			if(row.isObject) {
				try {
					...
					console.warn(`Error parsing JSON in ${this.table()}.${row.field}#${data.id}: ${ex} - original json:\n\n<<${data[row.field]}>>\n\n`);
				}
			} else
	...
	static async fromSql(whereClause, args) {
		...
		const sql = 'select * from ' + autoFixTable(this.table(), dbh) + ' where ' + whereClause;
...
const loadDefinition = definitionFile => {
	const basePath = path.dirname(parentModule()),
		definition = typeof(definitionFile) === 'function'
			? definitionFile
			: require(path.join(basePath, definitionFile)),
		schema = convertDefinition(definition);
...
module.exports = { loadDefinition, DatabaseObject, convertDefinition, QueryTiming };
