...
const { defineModel, applyTableNames } = require('./model/define-model');
const {
	stripManagedKeys,
...
const { LOADED_AT, readIssuedAt } = require('./model/loaded-at');
const {
	recordSet,
	takeUnsavedSets,
	supersededSets,
...
class DatabaseObject {
	...
	static MODEL_EXTENSIONS = [...MODEL_EXTENSIONS];
	...
	static async fromSql(
		...
		return this.retryIfConnectionLost((dbh) => {
			...
			const loadedAt = readIssuedAt();
			return dbh.roQuery(sql, args).then((rows) => {
				return promisePoolMap(
					rows,
					async (row) => this.inflate(row, span, undefined, { loadedAt }),
					promisePoolMapConfig,
				);
			});
		});
	...
	static async search(
		...
		const loadedAt = readIssuedAt(tx);
		...
		if (limitOne) {
			return this.inflate(res, span, undefined, { tx, loadedAt });
		}

		return promisePoolMap(
			res,
			async (object) => this.inflate(object, span, undefined, { tx, loadedAt }),
			effectivePoolConfig,
		);
	}
	...
	/**
	 * Set a given `field` to a `newValue`, and save it `PATCH_DEFER_DELAY`
	 * (300ms) later: one `this.update(changes)` for every `set()` in that time.
	 * Until that save, a read of the row doesn't undo the new value, and a
	 * `patch()` of the same field wins (it is the later write). A field not in
	 * the schema is only assigned. Nothing awaits the save: its errors go to
	 * `onAutoSaveError`. The save runs outside any transaction: inside one, use
	 * `patch(data, { tx })`. See lib/model/auto-save.js.
	 * @param {string|object} field Field to set, or an object of fields and values
	...
	set(field, newValue) {
		...
		this[field] = newValue;
		if (this.constructor.schema().fieldMap[field]) {
			recordSet(this, field, newValue);
			this._deferPatch();
		}
	...
	_deferPatch() {
		...
		this._patchDeferTid = setTimeout(() => {
			const changes = takeUnsavedSets(this);
			if (!Object.keys(changes).length) {
				// A patch() of those fields already wrote them.
				return;
			}
			Promise.resolve()
				.then(() => runAutoSave(this, () => this.update(changes)))
	...
	async patchIf(values = {}, ifFalsey = {}) {
		// As patch(): what it writes is the later write for set() edits of those
		// fields. It compares with the instance, unsaved edits included, so only
		// the fields it really wrote supersede them.
		const superseded = supersededSets(this, [
			...Object.keys(values),
			...Object.keys(ifFalsey),
		]);

		const span = { name: 'patchIf', props: { values, ifFalsey }, stack: [] };

		let dbhUsed;
		const row = await this.retryIfConnectionLost((dbh) => {
			dbhUsed = dbh;
			return dbh.patchIf(
		...
		dropSupersededSets(this, superseded, realDiff);
		const updated = await this._updateProperties(row, span);
		await emitChange({
	...
	async patch(data, { tx } = {}) {
		// The later write: once written, set() edits of these fields are not saved.
		const superseded = supersededSets(this, Object.keys(data || {}));
		...
		const row = await this._runOn(tx, (dbh) =>
			dbh.patch(this.constructor.table(), this[this.idField()], deflated),
		);
		dropSupersededSets(this, superseded);
		const updated = await this._updateProperties(row, span, { tx });
	...
	/**
	 * So named so it's not easy to delete accidentally. Fires the committed
	 * change hooks (`wasDeleted: true`), but not the global change hooks or
	 * afterChangeHook, which never saw a hard delete.
	 */
	async reallyDelete() {
		this.constructor.removeCachedId(this[this.idField()]);
		// No save of set() edits to a row that is gone.
		clearTimeout(this._patchDeferTid);
		clearUnsavedSets(this);

		const { table } = parseIdField(this.constructor.table());
		const result = await this.retryIfConnectionLost((dbh) => {
			const idField = this.idField();
...
module.exports = {
	loadDefinition,
	// A model from an inline schema, its types inferred (no codegen), and
	// renaming defined models' tables at startup
	defineModel,
	applyTableNames,
	prefixedId,
	timeOrderedId,
	...
	registerGlobalChangeHook,
	// ... and once those (and reallyDelete()) are committed
	registerCommittedChangeHook,
	// Transaction lifecycle, for code that acts once a transaction ends
	onTransactionEnd,
	transactionLocal,
	// When an instance's data was read (performance.now()), a hidden property
	LOADED_AT,
	...
	getRegisteredModel,
	checkLinks,
	// Dialect helpers for raw SQL (upsert with a condition, IN a list, read
	// back after a write, row locks, the clock and intervals, ...)
	sqlHelpers,
};
