...
class DatabaseObject {
	...
	static _getClassCache() {
		const key = this._classCacheKey();
		const cache =
			OBJECT_INSTANCE_CACHE[key] || (OBJECT_INSTANCE_CACHE[key] = {});
		return cache;
	}
	...
	static _classCacheKey() {
		...
		if (!Object.prototype.hasOwnProperty.call(this, CLASS_CACHE_KEY_SYMBOL)) {
			let table = '';
			try {
				table = this.table();
			} catch (err) {
			...
			this[CLASS_CACHE_KEY_SYMBOL] = `${this.name}:${table}`;
		}
		return this[CLASS_CACHE_KEY_SYMBOL];
	}
	...
	static async setCachedId(id, freshData) {
		...
		if (cached) {
			this._freshenInstance(cached, freshData);
			return cached;
		}
	 ...
	 */
	static clearCache() {
		OBJECT_INSTANCE_CACHE[this._classCacheKey()] = {};
	}
	...
	static async inflate(
		...
		let cached = txCache
			? txCache.get(this, id)
			: await this.getCachedId(id, span);

		if (!cached) {
			cached = new this({ id, [idField]: id }, FROM_INFLATE_SYMBOL);
			if (txCache) {
				txCache.set(this, id, cached, { created: true });
			} else {
				// 'await' so we can allow subclasses to do async work and block if needed
	...
	static async _resolveModelClass(modelName, errorHint = '', span = undefined) {
		...
		let resolvedPath = PATH_CACHE.get(pathCacheKey);
		if (!resolvedPath) {
			...
			PATH_CACHE.set(pathCacheKey, resolvedPath);
		}
	...
	async _updateProperties(data, span, { tx } = {}) {
		...
		const txCache = txInstanceCache(tx);
		if (txCache) {
			txCache.set(this.constructor, id, this, { written: true });
		} else {
			// 'await' so we can allow subclasses to do async work and block if needed
