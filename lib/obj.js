...
class DatabaseObject {
	...
	static async _resolvedLinkedModel(modelName, modelId) {
		let ModelClass = require(path.resolve(this.basePath(), modelName));
		...
		if (this.constructor.schema().fieldMap.nonce) {
			...
			if (diskNonce && diskNonce !== memoryNonce) {
				const error = new Error(
					`Nonce mismatch for table ${table} id ${
						this.id
					} (disk='${diskNonce}', memory='${memoryNonce}') - reload data from disk and retry patch. Wanting to patch: ${JSON.stringify(
						data,
					)}`,
				);
