...
import { convertDefinition } from './def-to-schema.js';
import { finder } from './finder.js';
import { parseIdField } from './parseIdField.js';
 ...
 */
class DatabaseObject {
	// Used to cache and lookup models by name
	static modelClassResolution = {};
	...
	static async _resolvedLinkedModel(modelName, modelId, span = undefined) {
		let ModelClass = this.modelClassResolution[modelName];
		if (!ModelClass) {
			...
			ModelClass = require(resolvedPath);

			if (ModelClass.schema) {
				ModelClass = ModelClass.schema; // support for 'export const schema=({types:t})=>{...};
			} else if (ModelClass.default) {
				ModelClass = ModelClass.default; // es6 export as default
			}

			// Cache resolution so we don't have to resolve again in the future
			this.modelClassResolution[modelName] = ModelClass;
		}
	...
	static async withDbh(callback, props) {
		...
		if (typeof callback === 'string' && props) {
			const sql = callback;
	...
	static async findOrCreate(fields, patchIf = {}, patchIfFalsey = {}) {
		...
		if (wasCreated && this.schema().fieldMap.createdAt) {
...
const loadDefinition = (definitionFile) => {
	const basePath = url.fileURLToPath(path.dirname(parentModule()));

	// const definition =
	// 	typeof definitionFile === 'function'
	// 		? definitionFile
	// 		: await import(path.resolve(basePath, definitionFile));

	const filePath =
		typeof definitionFile === 'string' &&
		path.resolve(basePath, definitionFile);

	// console.log(`loadDef debug`, {
	// 	basePath,
	// 	definitionFile,
	// 	filePath,
	// 	// definition,
...
export { convertDefinition };
export { retryIfConnectionLost };
export { QueryTiming };
export { QueryLogger };
export { libUtils };
export { dbhUtils };
