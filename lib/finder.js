...
// Quoting, LIMIT and the null fallback come from the dialect, so find() runs
// on Postgres (and SQLite) as well as MySQL. The SQL on MySQL is unchanged.

// IDEMPOTENT by contract: dbQuote(dbQuote(x)) === dbQuote(x). BDL-3893 -- both
// field-equality branches below used to pre-quote the field name and then hand
// it to prefixedField(), which quotes both of its arguments again, emitting
// `table`.``field`` and failing with MySQL errno 1064 on every call. dbQuote
// is published on the hook context (see hookCtx), so third-party code reaches
// it and cannot be relied on to know whether a value is quoted yet. See
// BaseDialect.quoteIdentifierOnce().
function dbQuote(identifier, dialect) {
	return dialect.quoteIdentifierOnce(identifier);
}

function prefixedField(table, field, dialect) {
	return `${dbQuote(table, dialect)}.${dbQuote(field, dialect)}`;
}

function getStringifyColumns(def, opts, dialect) {
	opts = opts || (opts = {});
	...
	rawFields.forEach((origFieldName) => {
		...
		if (origFieldName.startsWith('#')) {
			...
			const sql = opts.lower
				? `LOWER(${prefixedField(def.table, fieldName, dialect)})`
				: prefixedField(def.table, fieldName, dialect);

			sqlList.push(dialect.ifNullSql(sql, "''"));
		} else if (!opts.onlyCols) {
			const str = prefixedField(
				def.table,
				origFieldName.replace("'", "\\'"),
				dialect,
			);
...
function getStringifySql(def, opts, dialect) {
	opts = opts || (opts = {});
	const sqlList = getStringifyColumns(def, opts, dialect);

	const sql =
		sqlList.length > 1
			? dialect.concatSql(sqlList)
			: sqlList.length === 1
			? sqlList[0]
			: "''";
...
async function finder(
	params,
	{
		promisePoolMapConfig = { concurrency: 5, yieldEvery: 3 },
		...incomingOpts
	} = {},
) {
	const model = this;
	...
	// A handle without a dialect (a stub): the configured one.
	const dialect = conn.dialect || getDialect(config.dialect);
	const quote = (identifier) => dbQuote(identifier, dialect);
	const field = (table, name) => prefixedField(table, name, dialect);
	...
	const sqlData = {
		...
		whereArgs: [],
		whereList: [],
		tableName: quote(def.table),
	};

	// Just for debugging
	sqlData.fieldList.push(`${quote(def.table)}.*`);

	// Initalize where clause
	sqlData.whereList = [
		def.legacyExternalSchema
			? 'deleted=0'
			: `${field(def.table, 'isDeleted')} = ${dialect.toBooleanLiteral(false)}`,
	];

	// Basic implicit filters - match account if given
	if (where.account !== undefined) {
		if (where.account === null) {
			sqlData.whereList.push(`${quote('account')} = null`);
		} else {
			sqlData.whereList.push(`${quote('account')} = ?`);
	...
	if (query.q) {
		// match_ratio() is a stored function schema sync installs on MySQL only.
		if (!dialect.supportsStoredFunctions) {
			throw new Error(
				`[yass-orm] find({ q }) ranks with match_ratio(), a stored function yass installs on MySQL only; the ${dialect.name} dialect has none`,
			);
		}
		...
		// (1) Add Ranking Column using the match_ratio() function from AppCore
		{
			...
			const columns = getStringifyColumns(def, { onlyCols: true }, dialect);
			...
			if (columns.length > 1) {
				...
				const textSql = getStringifySql(def, { lower: true }, dialect); // lower: lower case the fields
			...
			sqlData.fieldList.push(
				`${getStringifySql(def, { lower: false }, dialect)} as _search_text`,
			);
		}
		...
		// Shape only: fieldArgs/whereArgs ARE the caller's search string (BDL-3886)
		if (isDebugEnabled('finder')) {
			const { fieldList, fieldArgs, whereList, whereArgs } = sqlData;
			console.log(`finder.js: After processing query.q`, {
				fieldList,
				whereList,
				fieldArgCount: fieldArgs.length,
				whereArgCount: whereArgs.length,
			});
		}
	}
	...
	function addSortingField(datum) {
		...
		sqlData.orderFieldList.push(`${field(def.table, fieldName)} ${direction}`);
	}
	...
	const hookCtx = {
		...
		dbQuote: quote,
		def,
		getStringifySql: (d, o) => getStringifySql(d, o, dialect),
		getStringifyColumns: (d, o) => getStringifyColumns(d, o, dialect),
		debugSql,
		queryValues,
		queryValuesPlain,
		filters,
	...
	return opts
			...
			if (opts.queryParams) {
				opts.queryParams.forEach((fieldName) => {
					...
					if (value !== undefined && value !== null) {
						// BARE name: prefixedField() quotes both arguments itself.
						// Pre-quoting here emitted `table`.``field`` (BDL-3893).
						sqlData.whereList.push(`${field(def.table, fieldName)} = ?`);
				...
				Object.keys(def.fieldMap).forEach((fieldName) => {
					...
					if (value !== undefined && value !== null) {
						// BARE name: prefixedField() does the quoting (BDL-3893).
						sqlData.whereList.push(`${field(def.table, fieldName)} = ?`);
						sqlData.whereArgs.push(value);
					}
				});
		...
		.then(async ([joinSql, joinArgs] = []) => {
			...
			if (filters.$limit)
				sql += `\n${dialect.limitSql(filters.$limit, filters.$skip)}`;
			...
			// Placeholder SQL and key names only — never the bound values (BDL-3886)
			if (isDebugEnabled('finder')) {
				console.log('******** generated:\n\n', sql, '\n', {
					boundArgCount: sqlData.fieldArgs.concat(whereArgs).length,
		...
		.then((packet) => {
			...
			if (
				...
				return queryValues(
					`COUNT(${field(def.table, idColumnSafe)}) AS totalRows`,
				).then((result) => {
					timeHelp.mark('packet.total');

					// Postgres folds the unquoted alias to lower case (it stays
					// unquoted so the SQL on MySQL is unchanged).
					const [{ totalRows, totalrows }] = result;
					packet.total = parseFloat(
						totalRows === undefined ? totalrows : totalRows,
					);
...
function filterData(results, parentCtx, sql, params, debugName) {
	...
	// Placeholder SQL only — ctx.debugSql would interpolate params (BDL-3886)
	if (isDebugEnabled('finder')) {
		console.log(
			`[custom-query-filter.filterData] (${debugName}): SQL:\n\n${sql}\n# Resulting ID count: ${idList.length}`,
		);
	}
