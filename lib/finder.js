...
// A well-formed backtick-quoted identifier, optionally table-qualified:
// `col` or `table`.`col`. Used to make dbQuote() idempotent.
...
// `table`.``field`` and failing with MySQL errno 1064 on every call. The call
// sites are fixed too, but the quoting itself has to be safe to apply twice:
// dbQuote is published on the hook context (see hookCtx), so third-party code
// reaches it and cannot be relied on to know whether a value is quoted yet.
function dbQuote(identifier) {
	const str = `${identifier}`;
	// Already quoted — return it untouched. Stripping and re-wrapping would
	// collapse `table`.`col` into the single, WRONG name `table.col`, which
	// reads silently rather than erroring.
	if (QUOTED_IDENTIFIER_RE.test(str)) {
		return str;
	}
	// Otherwise wrap, DROPPING any stray backticks so the result is always a
	// single well-formed pair that cannot be broken out of. Deliberately not
	// MySQL's escape-by-doubling: `a``b` contains interior backticks, so it
	// would fail the test above and gain another pair on the next call — i.e.
	// doubling and idempotency cannot both hold. Idempotency is the contract
	// callers here rely on, so a backtick inside an identifier is dropped.
			...
			if (opts.queryParams) {
				opts.queryParams.forEach((fieldName) => {
					...
					if (value !== undefined && value !== null) {
						// if(value === null) {
						// 	sqlData.whereList.push(
						// 		`${prefixedField(def.table, fieldName)} is null`,
						// 	);
						// } else {
