...
function finder(params, incomingOpts) {
	...
	sqlData.whereList = [
		def.legacyExternalSchema ? 'deleted=0' : '`isDeleted` = 0',
	];
	...
	return opts
		...
		.then(async (result) => {
			...
			result = await Promise.all(
				result.map(async (row) => {
					if (row._search_rank) row._search_rank = parseFloat(row._search_rank);
		...
		.then((packet) => {
			...
			if (
				...
				return queryValues(`COUNT(${idColumnSafe}) AS totalRows`).then(
					(result) => {
