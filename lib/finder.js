...
const { dbh: GetDbh, sqlEscape } = require('./dbh');

const dbh = GetDbh();

function dbQuote(identifier) {
	return  sqlEscape(identifier); //'`' + (identifier) + '`';
}
...
function getStringifyColumns(def, opts) {
	...
	// console.dir(def.fieldMap, { depth: 10 });
			...
			.forEach(row => {
				if(row.objectSchema) {
					Object.values(row.objectSchema).forEach(subrow => {
						if(subrow.type === 'varchar' ||
						   subrow.type === 'longtext') {
	...
	rawFields.forEach(origFieldName => {
		...
		if(origFieldName.startsWith('#')) {
			...
			const sql = opts.lower ?
				'LOWER('+dbQuote(fieldName)+')'
				: dbQuote(fieldName)
			;

			sqlList.push(`IFNULL(${sqlEscape(sql)},"")`);
		} else 
		if(!opts.onlyCols) {
			const str = '"' + origFieldName.replace('"', '\\"') + '"';

			sqlList.push(opts.lower ?
				'LOWER(' + str + ')'
				: str
			);
		}
...
function getStringifySql(def, opts) {
	...
	const sql =  sqlList.length > 1
		? "CONCAT(" + 
			sqlList.join(', ')
		+ ")"
		: sqlList.length == 1 ? sqlList[0]
		: "''";
...
function finder(params, opts) {
	...
	if(model.allowedFindParams)
		opts.queryParams = model.allowedFindParams();
	...
	sqlData.whereList = [def.legacyExternalSchema ? 'deleted=0' : '`isDeleted` = 0'];
	...
	if(query.q) {
		...
		// (1) Add Ranking Column using the match_ratio() function from AppCore
		{
			...
			columns.forEach(identifier => {
				ratio_list.push('match_ratio(' + identifier + ', ?, ?)');
			...
			if(columns.length > 1) {
				...
				ratio_list.push(`match_ratio(${textSql}, ?, ?)`);
				sqlData.fieldArgs.push(filterNonWild, filterWild);


				// # Combine them into an average match ratio as a single SQL statement,
				// # suitable for use like "select $rank_sql as `ranking` from ... where ...""
				const rank_sql = "((\n\t"
					+ ratio_list.join(" + \n\t")
					+ ") / " + ratio_list.length + ")";

				// fields is the list of fields to select
				sqlData.fieldList.push(`${rank_sql} as _search_rank`);

			} else {

				// Just select the single ratio for the rank since only one stringification value
				sqlData.fieldList.push(ratio_list[0] + ' as _search_rank');
			}

			// For returning to the user
			sqlData.fieldList.push(getStringifySql(def, { lower: false }) + ' as _search_text');
		}
	...
	function queryValues(fieldList, whereClause, whereArgs) {
		if(!whereClause)
			whereClause = '1=1 -- Empty whereClause on queryValues';
	...
	function addSortingField(datum) {
		...
		if(typeof(datum) !== 'string' && datum.length == 2) {
			fieldName =  datum[0];
			direction = (datum[1] + "").toUpperCase();

			if(['ASC','DESC'].indexOf(direction) < 0) {
				throw new Error("Unknown direction '"+direction+"' on field '"+fieldName+"'");
			}
		...
		sqlData.orderFieldList.push(dbQuote(fieldName)+' ' + direction);
	}

	const hookCtx = { addSortingField, query, dbh, dbQuote, def, getStringifySql, getStringifyColumns, debugSql, queryValues, queryValuesPlain, filters };
	...
	return opts.mutateQuery(query, sqlData, hookCtx).then( () => {
		...
		const orderSourceList = JSON.parse(JSON.stringify(order && order.length > 0 ? order : def.sortBy ?  def.sortBy : []));

		// Add our search ranking to the sort list
		if(query.q)
			orderFieldList.push('_search_rank DESC');
		...
		sqlData.orderClause = sqlData.orderFieldList.length ? sqlData.orderFieldList.join(', ') : 'id';
		sqlData.tableName = dbQuote(def.table);
		...
		if(filters.$limit)
			sql += "\n"
				+ "LIMIT "
				+ parseInt(filters.$skip || 0, 10)
				+ ", "
				+ parseInt(filters.$limit, 10);

		// console.log("****** generated: ", sql, sqlData.fieldArgs, sqlData.whereArgs);
		
		// const t1 = new Date();
		const res = dbh.pquery(sql, sqlData.fieldArgs.concat(sqlData.whereArgs));
		// const t2 = new Date(), delta = t2 - t1;

		// console.log("******** generated:\n\n", debugSql(sql, sqlData.fieldArgs.concat(sqlData.whereArgs)), "\n", {query, filters, delta});
		...
		if(filters.$limit && !packet.extra.$infiniteTotal && !packet.totalSetManually) {
			// return dbh.pquery(
			// `SELECT COUNT(id) AS totalRows
			//  FROM   ${sqlData.tableName}
			//  WHERE  ${sqlData.whereClause}`, {
			// 	replacements: sqlData.whereArgs,
			// 	type: dbh.pqueryTypes.SELECT
			// }).then(result => {
			// 	packet.total = result[0].totalRows;
			// 	return packet;
			// });

			return queryValues(`COUNT(${idColumnSafe}) AS totalRows`)
				.then(result => {
...
function filterData(results, parentCtx, sql, params, debugName) {
	const idList = [];
	results.forEach(results => idList.push(results.id));
	...
	console.log("[custom-query-filter.filterData] ("+debugName+"): SQL:\n\n" + parentCtx.ctx.debugSql(sql,params)+"\n# Resulting ID list: ", idList.slice(0,5), "# only up to first 5 ids ");

	if(idList.length > 0) {
		parentCtx.data.whereList.push('id in ("' + idList.join('","') + '") -- Matches ' + debugName+' filter');
		// parentCtx.data.whereArgs.push(idList);
	} else {
...
function promiseFilter(sql, params, parentCtx, debugName) {
	const sequelize = parentCtx.ctx.sequelize;

	return dbh.pquery(sql, params)
		.then(results => filterData(results, parentCtx, sql, params, debugName));
}
