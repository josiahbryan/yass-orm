...
function generateModelType(convertedSchema) {
	...
	// Generate model type with prettier-compatible multi-line method signatures
	return `/**
 ...
 *
 * Smart output based on model file type:
 * - If model is TypeScript (.ts): output to defs/ (avoids .ts/.d.ts conflict)
 * - If model is JavaScript (.js): output next to model (TypeScript finds it)
 *
 * @param {string} definitionPath - Path to model definition .js file
...
function determineOutputPath(definitionPath) {
	...
	if (path.basename(dir) === 'defs') {
		...
		// If TypeScript model exists, keep types in defs/ to avoid conflict
		if (fs.existsSync(tsModelPath)) {
			return resolvedPath.replace(/\.js$/, '.d.ts');
		}

		// If JS model exists, put types next to it for auto-discovery
