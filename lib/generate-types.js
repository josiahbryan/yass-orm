...
function mapFieldToZodSchema(
	...
	const buildValidationChain = (baseSchema, fieldData, schemaType) => {
		...
		if (fieldData._description) {
			...
			const escaped = fieldData._description.replace(/'/g, "\\'");
			chains.push(`.describe('${escaped}')`);
		}
	...
	if (
		...
		if (field._description) {
			const escaped = field._description.replace(/'/g, "\\'");
			return `${baseSchema}.describe('${escaped}')`;
		}
		return baseSchema;
	}
