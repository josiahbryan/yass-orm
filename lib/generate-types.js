...
/**
 * Render arbitrary text as a JS/TS string LITERAL that is guaranteed to parse.
 *
 * BDL-2852: this used to be hand-rolled as `text.replace(/'/g, "\\'")` inside a
 * single-quoted template. That handles apostrophes and nothing else, so any
 * description containing a raw newline terminated the literal mid-string and the
 * whole generated file failed to parse (TS1002 unterminated string literal, then
 * a cascade of TS1005/TS1127/TS1434).
 *
 * `JSON.stringify` is the right tool here rather than more hand-rolled quoting:
 * it escapes newlines, carriage returns, tabs, double quotes, backslashes and
 * control characters in one pass, and its output is a valid JS string literal by
 * construction. Hand-rolled escaping is exactly where this class of bug lives —
 * a pass that handles `\n` but not `\` still lets `\` + `'` close the string.
 *
 * The two extra replacements cover U+2028/U+2029, which are legal inside a JSON
 * string but are JS LineTerminators; JSON.stringify emits them raw, so escape
 * them explicitly. (ES2019 legalized them in string literals, but older
 * downstream parsers still choke, and there is no cost to being correct.)
 *
 * @param {string} text - arbitrary text
 * @returns {string} a source-code string literal, quotes included
 */
...
/**
 * Split arbitrary text into lines that are safe to embed inside a JSDoc block
 * comment.
 *
 * Two hazards, both from the same "the emitted artifact must be correct for
 * ARBITRARY input" rule as toJsStringLiteral():
 *  - an end-of-comment sequence (asterisk-slash) in the text would terminate the comment early, and
 *    everything after it becomes (invalid) code;
 *  - raw newlines must become properly-prefixed JSDoc continuation lines rather
 *    than dangling unprefixed text.
 *
 * @param {string} text - arbitrary text
 * @returns {string[]} one entry per line, comment-terminator neutralized
 */
function toJsDocLines(text) {
	return String(text)
		.replace(/\*\//g, '*\\/')
		.split(/\r\n|\r|\n/);
}
...
/**
 * Quote an enum's option values into TS string-literal members, DROPPING any
 * `null`/`undefined` entry.
 *
 * A def may list `null` as an enum value to make it the default via
 * `t.enum([null, 'a', 'b'], { defaultValue: null })` (yass-orm uses the first
 * value as the column default). Interpolating that `null` naively (`` `'${v}'` ``)
 * would emit the STRING-LITERAL type `'null'` — a bogus union member that then
 * fails to satisfy every consumer expecting the real value union. Every enum
 * type this generator emits already appends `| null` for nullability, so a `null`
 * option is redundant there anyway: filter it out and the union stays the clean
 * set of real values (+ the trailing `| null`).
 *
 * @param {Array<string|null|undefined>} options - raw enum option values
 * @param {(literal: string) => string} [format] - wraps each `'value'` literal
 *   (e.g. to add multi-line `| ` prefixes); defaults to identity.
 * @returns {string[]} formatted TS string-literal members (null/undefined removed)
 */
function enumLiteralMembers(options, format = (literal) => literal) {
	return options.filter((v) => v != null).map((v) => format(`'${v}'`));
}
 ...
 *
 * @param {string} str - Plural string (e.g., 'chat_inboxes', 'users')
 * @returns {string} Singular form (e.g., 'chat_inbox', 'user')
 */
function singularize(str) {
	// Rule 1: Words ending in -xes, -sses, -ches, -shes, -zes → drop -es
	if (/[xsz]es$/.test(str) || /[cs]hes$/.test(str)) {
		return str.slice(0, -2);
	}
	// Rule 2: Words ending in -ies → change to -y
	if (/ies$/.test(str)) {
		return `${str.slice(0, -3)}y`;
	}
	// Rule 3: Default - drop trailing -s
...
function mapFieldToTsType(
	...
	// A lazy reference, t.linked(() => Model): no path to import a type from.
	if (typeof linkedModel === 'function') {
		return 'unknown';
	}
	...
	if (
		...
		return `${enumLiteralMembers(options).join(' | ')} | null`;
	}
	...
	if (isArray) {
		...
		if (
			...
			const enumType = `${enumLiteralMembers(arrayItemEnumOptions).join(
				' | ',
			)} | null`;
	...
	// Map SQL types to TS types (a sized varchar, e.g. a stringLinkedIds link
	// column, is a varchar)
	switch (/^varchar\(\d+\)$/i.test(`${type}`) ? 'varchar' : type) {
		case 'uuidKey':
		case 'char(36)':
		case 'varchar':
		case 'longtext':
...
function generateSubTypeInterfaces(subTypes, linkedModelImports, outputDir) {
	...
	// Helper to generate JSDoc for sub-type fields
	const generateFieldJsDoc = (fieldDef) => {
		const description = fieldDef._description;
		const example = fieldDef._example;
		...
		const lines = [];
		if (description) {
			lines.push(...toJsDocLines(description));
		}
		if (example !== undefined) {
			lines.push(...toJsDocLines(`@example ${JSON.stringify(example)}`));
		}
	...
	for (const [typeName, subType] of subTypes) {
		...
		const fieldDefs = Object.entries(fields)
			.map(([key, fieldDef]) => {
				...
				const jsdoc = generateFieldJsDoc(fieldDef);
				return `${jsdoc}\t${actualKey}?: ${tsType}`;
			})
			.join(';\n');
...
function generateInstanceInterface(
	...
	const baseName = toPascalCase(singularize(table));
	...
	// Helper to generate JSDoc comment from field metadata
	const generateJsDoc = (fieldData, fallbackComment = null) => {
		const description = fieldData._description;
		const example = fieldData._example;

		if (!description && !example && !fallbackComment) {
			return '';
		}
		...
		if (description) {
			lines.push(...toJsDocLines(description));
		} else if (fallbackComment) {
			lines.push(...toJsDocLines(fallbackComment));
		}
		if (example !== undefined) {
			lines.push(...toJsDocLines(`@example ${JSON.stringify(example)}`));
		}
	...
	const fieldDefs = fields
		.map((field) => {
			...
			// A lazy reference, t.linked(() => Model): codegen reads the
			// definition, not the model, so it can't see the target's type.
			if (typeof linkedModel === 'function') {
				const jsdoc = generateJsDoc(
					field,
					'Linked model (lazy reference; its type is not generated)',
				);
				return `${jsdoc}\t${fieldName}: unknown`;
			}
			...
			if (linkedModel) {
				...
				if (isBareModuleName && definitionDir) {
					...
					if (resolvedPath) {
						...
						const jsdoc = generateJsDoc(
							field,
							`Linked ${className} (unresolved: ${linkedModel})`,
						);
						return `${jsdoc}\t${fieldName}: unknown`;
					}
				}
				...
				// Use custom description if provided, else fallback to "Linked ClassName"
				const jsdoc = generateJsDoc(field, `Linked ${className}`);
				return `${jsdoc}\t${fieldName}: ${typeName}`;
			}
			...
			if (
				...
				const unionLines = enumLiteralMembers(
					options,
					(literal) => `\t\t| ${literal}`,
				).join('\n');
				return `${jsdoc}\t${fieldName}:\n${unionLines}\n\t\t| null`;
			}
			...
			// Generate JSDoc if we have description or example
			const jsdoc = generateJsDoc(field);
			return `${jsdoc}\t${fieldName}: ${tsType}`;
		})
...
export interface ${modelTypeName} {
	...
	search(
		...
		limitOne?: false,
		promisePoolMapConfig?: Record<string, unknown> & import('yass-orm').TxOptions,
		options?: import('yass-orm').TxOptions,
	 ...
	 * \`limitOne: true\` literal discriminates correctly (BDL-2646).
	 */
	search(
		query: Record<string, unknown>,
		options: import('yass-orm').SearchOptions & { limitOne: true },
		promisePoolMapConfig?: Record<string, unknown> & import('yass-orm').TxOptions,
		txOptions?: import('yass-orm').TxOptions,
	): Promise<${instanceName} | null>;

	/** Search with explicit bounds — always resolves to an ARRAY (BDL-2646) */
	search(
		query: Record<string, unknown>,
	 ...
	 *
	 * The second positional takes \`orderBy\`/\`orderDir\`, pool-config keys and
	 * \`tx\`; any other key throws naming itself (BDL-2700). It is deliberately
	 * NOT \`Record<string, unknown>\` — that open shape is what let a mistyped
	 * option through the compiler and into a silent runtime no-op.
	 */
	searchOne(
		query: Record<string, unknown>,
		options?: import('yass-orm').SearchOneOptions,
		txOptions?: import('yass-orm').TxOptions,
	): Promise<${instanceName} | null>;
	...
	get(
		id: string,
		opts?: { allowCached?: boolean; tx?: import('yass-orm').DbHandle },
	): Promise<${instanceName} | null>;
	...
	findOrCreate(
		...
		options?: {
			...
			transactionOptions?: import('yass-orm').TransactionOptions;
			tx?: import('yass-orm').DbHandle;
		},
	): Promise<${instanceName}>;

	/**
	 * Atomic at-most-once insert. Resolves to the created record, or \`null\`
	 * when a UNIQUE/PK conflict caused the insert to be SKIPPED — unlike
	 * findOrCreate, there is no search-then-create race window.
	 * \`conflictColumns\` is derived from the def's single unique index when
	 * omitted; pass \`uniqueIndex\` to disambiguate.
	 */
	createIgnore(
		data: Partial<${instanceName}>,
		options?: {
	...
	inflate(
		...
		span?: unknown,
		promisePoolMapConfig?: Record<string, unknown>,
		options?: import('yass-orm').TxOptions,
	): Promise<${instanceName} | null>;
}`;
}
...
function mapFieldToZodSchema(
	...
	// Helper to build validation chain from field metadata
	const buildValidationChain = (baseSchema, fieldData, schemaType) => {
		const chains = [];

		// String validations
		if (schemaType === 'string') {
			if (fieldData._minLength !== undefined) {
				chains.push(`.min(${fieldData._minLength})`);
			}
			if (fieldData._maxLength !== undefined) {
				chains.push(`.max(${fieldData._maxLength})`);
			}
			if (fieldData._pattern) {
				const pattern =
					fieldData._pattern instanceof RegExp
						? fieldData._pattern.toString()
						: `/${fieldData._pattern}/`;
				chains.push(`.regex(${pattern})`);
			}
			if (fieldData._format === 'email') {
				chains.push('.email()');
			}
			if (fieldData._format === 'url') {
				chains.push('.url()');
			}
		}
		...
		if (schemaType === 'number') {
			if (fieldData._min !== undefined) {
				chains.push(`.min(${fieldData._min})`);
			}
			if (fieldData._max !== undefined) {
				chains.push(`.max(${fieldData._max})`);
			}
			if (fieldData._positive) {
				chains.push('.positive()');
			}
			if (fieldData._negative) {
				chains.push('.negative()');
			}
			if (fieldData._nonnegative) {
				chains.push('.nonnegative()');
			}
		}

		// Array validations
		if (schemaType === 'array') {
			if (fieldData._minItems !== undefined) {
				chains.push(`.min(${fieldData._minItems})`);
			}
			if (fieldData._maxItems !== undefined) {
				chains.push(`.max(${fieldData._maxItems})`);
			}
		}

		// Description - applies to all types
		if (fieldData._description) {
			// Emit an escaped literal: description text is arbitrary (BDL-2852).
			chains.push(`.describe(${toJsStringLiteral(fieldData._description)})`);
		}

		return baseSchema + chains.join('');
	};

	// Linked fields are just UUIDs (string references)
	if (linkedModel) {
		return buildValidationChain('z.string()', field, 'string');
	}
	...
	if (
		...
		// Drop a `null` default-marker option (see enumLiteralMembers): the schema is
		// already `.nullable()`, and a literal 'null' member would validate the STRING
		// "null" as a legal value.
		const enumValues = enumLiteralMembers(options).join(', ');
		const baseSchema = `z.enum([${enumValues}]).nullable()`;
		// Add description if present
		if (field._description) {
			return `${baseSchema}.describe(${toJsStringLiteral(field._description)})`;
		}
		return baseSchema;
	}
	...
	if (isArray) {
		...
		if (
			...
			if (namedSchema) {
				baseSchema = `z.array(${namedSchema})`;
			} else {
			...
			const enumValues = enumLiteralMembers(arrayItemEnumOptions).join(', ');
			baseSchema = `z.array(z.enum([${enumValues}]).nullable())`;
		} else {
			// Simple arrays
			switch (arrayItemType) {
				case 'string':
					...
					break;
				case 'bigint':
					baseSchema = 'z.array(z.string().regex(/^-?\\d+$/))';
					break;
				case 'number':
				...
				case 'boolean':
					...
					break;
				case 'Date | null':
					baseSchema = 'z.array(z.coerce.date().nullable())';
					break;
				default:
		...
		return buildValidationChain(baseSchema, field, 'array');
	}
	...
	if (isObject) {
		...
		if (objectSchema && Object.keys(objectSchema).length > 0) {
			...
			if (namedSchema) {
				baseSchema = `${namedSchema}.nullable()`;
			} else {
				// Fall back to inline schema
				const subFields = Object.entries(objectSchema)
					.map(([key, subField]) => {
	...
	switch (/^varchar\(\d+\)$/i.test(`${type}`) ? 'varchar' : type) {
		case 'uuidKey':
		case 'char(36)':
			// A t.stringKey id is a uuidKey flagged stringId: a prefixed string
			// (`chat_0mfq...`), not a UUID.
			baseSchema = field.stringId ? 'z.string()' : 'z.string().uuid()';
			schemaType = 'string';
			break;
		...
		case 'datetime':
			baseSchema = 'z.coerce.date().nullable()';
			schemaType = 'datetime';
			break;
	...
	return buildValidationChain(baseSchema, field, schemaType);
}
