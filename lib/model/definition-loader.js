...
/**
 * The class loadDefinition() and defineModel() return: a subclass of the
 * configured `baseClass` (default DatabaseObject) whose schema is
 * `getSchema()`.
 *
 * @param {Object} options
 * @param {string} options.basePath Folder path links resolve from
 * @param {Function} options.getSchema Returns the converted schema
 * @param {string} [options.objectIdPrefix] Gives prefixed ids when set
 * @returns {Function} The model class
 */
const createModelClass = ({ basePath, getSchema, objectIdPrefix }) => {
	...
	if (objectIdPrefix) {
		ModelClass.generateObjectId = () => prefixedId(objectIdPrefix);
	}
...
const loadDefinition = (definitionFile) => {
	...
	return createModelClass({
		basePath,
		getSchema: () => schema,
		objectIdPrefix: schema.objectIdPrefix,
	});
};
