...
const { LOADED_AT, stampLoadedAt } = require('./loaded-at');
...
/**
 * Copies every schema field, and the LOADED_AT stamp, from `source` onto
 * `target`; `target`'s unsaved set() edits stay on top (see auto-save.js).
 */
const freshenInstance = (Model, target, source) => {
	Model.fields().forEach(({ field }) => {
		target[field] = source[field];
	});
	reapplyUnsavedSets(target);
	if (source[LOADED_AT] !== undefined) {
		stampLoadedAt(target, source[LOADED_AT]);
	}
};
