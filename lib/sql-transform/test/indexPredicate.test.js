...
describe('#indexPredicate (AST canonicalization)', () => {
	...
	// (`meta->>'$.x'`). Postgres reports it back with the path resolved to a bare key
	// (`meta ->> 'x'`), because the query transformer rewrites the DDL on the way out.
	// Without the same conversion here the two never matched and the index was
	// dropped and recreated on every sync -- the failure fixed for index COLUMNS in
	// 2.1.4, hiding in predicates.
	describe('JSON accessors inside a predicate', () => {
		it('matches a MySQL-spelled path against the resolved key Postgres reports', () => {
			expect(canon("meta->>'$.x' = 'y'")).to.equal(
				canon("((meta ->> 'x'::text) = 'y'::text)"),
			);
		});

		it('matches a NESTED path against the #>> form Postgres reports', () => {
			expect(canon("meta->>'$.a.b' = 'y'")).to.equal(
				canon("((meta #>> '{a,b}'::text[]) = 'y'::text)"),
			);
		});

		it('still distinguishes different JSON keys', () => {
			expect(canon("meta->>'$.x' = 'y'")).to.not.equal(
				canon("meta->>'$.z' = 'y'"),
			);
		});
	});
