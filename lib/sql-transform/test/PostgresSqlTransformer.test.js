...
describe('PostgresSqlTransformer', () => {
	...
	describe('? placeholders with an array of values', () => {
		it('turns ? into $1..$n when values are given', () => {
			const result = transformSqlWithScanner(
				'SELECT * FROM t WHERE a = ? AND b = ?',
				[1, 2],
			);
			expect(result.sql).to.equal('SELECT * FROM t WHERE a = $1 AND b = $2');
		});

		it('leaves a jsonb ? operator alone when the array is empty', () => {
			const sql = "SELECT * FROM t WHERE data ? 'k'";
			expect(transformSqlWithScanner(sql, []).sql).to.equal(sql);
			expect(transformSqlForPostgres({ sql, params: [] }).sql).to.include(
				"? 'k'",
			);
		});
	});
