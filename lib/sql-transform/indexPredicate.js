...
const { convertJsonAccessorsToPostgres } = require('./postgresJsonPath');

// node-sql-parser is an optional peer dependency. Only a PARTIAL index reaches
// this module, and only on Postgres or SQLite (MySQL has no partial indexes), so
// the parser is loaded the first time a predicate is compared.
let parser;
const getParser = () => {
	if (!parser) {
		const { Parser } = requireOptional('node-sql-parser', {
			feature: 'comparing a partial index predicate',
		});
		parser = new Parser();
	}
	return parser;
...
function canonicalizeIndexPredicateViaAst(predicate, database = 'postgresql') {
	...
	// A predicate can contain a JSON accessor, and schemas spell those MySQL-style
	// (`meta->>'$.x'`). Postgres reports the predicate back with the path resolved to
	// a bare key (`meta ->> 'x'`), because the query transformer rewrites the DDL on
	// its way out. Without applying the SAME conversion here, the desired side keeps
	// `'$.x'`, the two never compare equal, and the index is dropped and recreated on
	// every sync -- the identical failure mode fixed for index COLUMNS in 2.1.4,
	// hiding in predicates. Uses the shared converter, so the two cannot drift.
	const prepared = spellPatternOperatorsAsKeywords(
		convertJsonAccessorsToPostgres(predicate),
	);
	...
	// Outside the try below: a missing parser is an install problem to report,
	// not a predicate to fall back from.
	const sqlParser = getParser();

	for (let i = 0; i < attempts.length; i += 1) {
		try {
			const ast = sqlParser.astify(`SELECT 1 FROM t WHERE ${prepared}`, {
				database: attempts[i],
			});
