...
// Optional peer dependency: this module is loaded on the dialect's first query.
const { Parser } = requireOptional('node-sql-parser', {
	feature: 'the PostgreSQL dialect',
});
const {
	jsonPathToPostgres,
...
function transformAstForPostgres(ast) {
	return visitAst(ast, (node) => {
		...
		// JSON operators: convert a MySQL JSONPath to the Postgres accessor.
		// A single-level path keeps `->>`/`->` with a bare key; a NESTED path needs
		// the different operator `#>>`/`#>` with a text[] path literal. Stripping
		// `$.` alone turned `$.a.b` into the single key `a.b`, which no row has --
		// silently returning NULL rather than erroring.
		if (
			...
			if (
				...
				const { operator, keys } = jsonPathToPostgres(
					node.right.value,
					node.operator,
				);
				return {
					...
					right: {
						...node.right,
						// For a nested path the literal is a text[] path: {a,b}
						value: keys.length > 1 ? `{${keys.join(',')}}` : keys[0] || '',
...
function transformSqlWithScanner(sql, params = {}) {
	// An array of values goes with `?` placeholders (finder.js and its hooks
	// write them), which become $1, $2, ... in order. Not when the SQL already
	// has $N placeholders: then it was written for Postgres, and a `?` in it
	// is the jsonb operator. (A `$5` inside a string literal is not one.) Nor
	// when the array is empty: no values, so a `?` is the jsonb operator too.
	const tokens = tokenizeSql(sql);
	const positional = Array.isArray(params);
	let positionalCount = 0;
	const convertQuestionMarks =
		positional &&
		params.length > 0 &&
		!tokens.some((token) => token.type === 'code' && /\$\d/.test(token.value));
	const keys = positional
		? []
		: Object.keys(params || {}).sort((a, b) => b.length - a.length);
	...
	// Both quote styles, single- and multi-level paths, via the shared converter
	// the index DDL generator also uses -- so an index expression and a query
	// expression are guaranteed to come out identical.
	const transformed = convertJsonAccessorsToPostgres(parts.join(''));
